...
import { type SseEnvelope, sseEnvelopeSchema } from '@slacato/contracts';
...
export const ZodResponse =
  <TOutput>(schema: ZodType<TOutput>): MethodDecorator =>
  (_target, _propertyKey, descriptor) => {
    const handler = descriptor.value;
    if (!handler) throw new TypeError('ZodResponse requires a method descriptor');
    const existing = Reflect.getMetadata(WIRE_CONTRACT_METADATA, handler) as
      | WireContract
      | undefined;
...
export const ZodSseEnvelope =
  (schema: ZodType<unknown> = sseEnvelopeSchema): MethodDecorator =>
  (_target, _propertyKey, descriptor) => {
    const handler = descriptor.value;
...
export const validateSseEnvelope = (input: unknown): SseEnvelope =>
  sseEnvelopeSchema.parse(input) as SseEnvelope;
...
export const createSsePublisher =
  <TResult>(emit: (envelope: SseEnvelope) => TResult) =>
  (input: unknown): TResult =>
    emit(validateSseEnvelope(input));

/** Records a request-part contract on the controller handler before applying Nest's parameter decorator. */
function requestPart(
  part: RequestPart,
  declaration: RequestContract,
  nestDecorator: ParameterDecorator
): ParameterDecorator {
  return (target, propertyKey, parameterIndex) => {
    ...
    if (typeof handler !== 'function')
      throw new TypeError('Wire request schemas require a controller method');
    const existing = Reflect.getMetadata(WIRE_CONTRACT_METADATA, handler) as
      | WireContract
      | undefined;
    ...
    const nextContract: WireContract = existing?.response
      ? existing.sse
        ? { request, response: existing.response, sse: existing.sse }
        : { request, response: existing.response }
      : existing?.sse
        ? { request, sse: existing.sse }
        : { request };
