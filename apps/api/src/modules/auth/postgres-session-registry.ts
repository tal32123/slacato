...
export class PostgresSessionRegistry implements SessionRegistry {
  ...
  public async revoke(version: string): Promise<void> {
    await this.database.sql`update auth_sessions set revoked_at = coalesce(revoked_at, now()) where version = ${version}`;
  }

  public async isActive(version: string, userId: string): Promise<boolean> {
    const row = (await this.database.sql<{ active: boolean }[]>`select exists (
