...
export function EvidenceDetail({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: Selection changes must refocus the panel.
  useEffect(() => {
  ...
  if (desktop)
    return (
      ...
      >
        <EvidenceHeader onClose={onClose} />
...
function ModalEvidence({
  evidence,
  onClose
}: Readonly<{ evidence: EvidenceDetailView; onClose: () => void }>): React.JSX.Element {
  useEffect(() => {
        ...
        <SheetHeader className="sticky top-0 z-10 border-b bg-background px-5 py-5 text-left">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <SheetDescription>
                Authorized source record and stable citation identifiers.
              </SheetDescription>
            </div>
            <SheetClose asChild>
              <Button variant="outline" size="icon" aria-label="Close evidence detail">
                <X aria-hidden="true" />
              </Button>
            </SheetClose>
          </div>
...
function EvidenceHeader({ onClose }: Readonly<{ onClose: () => void }>): React.JSX.Element {
  return (
    <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b bg-card px-5 py-5">
      ...
      <Button variant="outline" size="icon" aria-label="Close evidence detail" onClick={onClose}>
        <X aria-hidden="true" />
      </Button>
    </header>
  );
  ...
  return (
    <div className="grid gap-6 px-5 py-6">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <p className="text-sm font-semibold break-words">{evidence.citationLabel}</p>
      ...
      <section aria-labelledby="source-record-title">
        <h3 id="source-record-title" className="text-sm font-semibold">
          Authorized source record
        </h3>
        <pre className="mt-3 whitespace-pre-wrap break-words rounded-lg bg-secondary p-4 font-sans text-sm leading-6 text-secondary-foreground">
          {evidence.content}
        </pre>
      </section>
    </div>
