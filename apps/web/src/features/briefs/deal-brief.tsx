  ...
  return (
    <article data-deal-main className="min-w-0">
      ...
      <header className="mt-3 border-b pb-7">
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><StatusBadge status={deal.restricted ? 'attention' : 'ready'} label={deal.restricted ? 'Restricted — authorized' : 'Authorized workspace'} /><StatusBadge status="readonly" label="Source snapshot" /></div>
            ...
            <p className="mt-3 max-w-3xl leading-7 text-muted-foreground">Authorized deal evidence for {deal.accountName}. Generated output, when available, remains separate and requires seller judgment.</p>
          </div>
          {primaryAction}
        </div>
      </header>
      ...
      <section className="border-b py-8" aria-labelledby="source-snapshot">
        <h2 id="source-snapshot" className="text-2xl font-semibold">{sourceSnapshot.label}</h2>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">Evidence overview assembled deterministically from currently authorized, ingested records. It is not AI-generated and is not produced by a run.</p>
        <WorkspaceContent brief={sourceSnapshot.evidenceOverview} evidence={evidence} selectedEvidenceId={selectedEvidenceId} onEvidence={onEvidence} sourceCues />
      </section>
      {generatedOutput !== null && <section className="border-b py-8" aria-labelledby="generated-output">
        <div className="flex flex-wrap items-center justify-between gap-3"><h2 id="generated-output" className="text-2xl font-semibold">{generatedOutput.lifecycle === 'finalized' ? 'Finalized generated output' : 'Generated draft'}</h2><StatusBadge status="readonly" label={generatedOutput.lifecycle === 'finalized' ? 'Finalized' : 'Draft'} /></div>
        <p className="mt-3 text-sm leading-6 text-muted-foreground">Produced by run {generatedOutput.producingRun.id} · {generatedOutput.producingRun.status.replaceAll('_', ' ')}</p>
        <WorkspaceContent brief={generatedOutput.content} evidence={evidence} selectedEvidenceId={selectedEvidenceId} onEvidence={onEvidence} />
      </section>}
    </article>
  );
}

function WorkspaceContent({ brief, evidence, selectedEvidenceId, onEvidence, sourceCues = false }: Readonly<{
  ...
  return <div>{sectionOrder.map((id) => {
    const section = brief.sections[id];
    const isSourceCue = sourceCues && (id === 'recommendedNextActions' || id === 'confidenceAndReviewWarnings');
    return <section key={id} className="border-b py-8" aria-labelledby={`${brief.status}-${id}`}>
      <div className="flex flex-wrap items-center justify-between gap-3"><h3 id={`${brief.status}-${id}`} className="text-xl font-semibold">{section.title}</h3><div className="flex flex-wrap gap-2">{isSourceCue && <StatusBadge status="readonly" label="Deterministic source cue" />}{section.accountTeamUpdateImpact && <StatusBadge status="attention" label="Account-team update impact" />}</div></div>
      ...
      {id === 'recommendedNextActions' && <Actions actions={brief.actions} sourceCues={sourceCues} />}
