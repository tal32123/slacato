...
export const approvalsQueryOptions = (version: string) =>
  scopedQuery(version, 'approvals', (signal) => fetchApprovals(signal));

/** Defines session-aware loading for a selected approval request. */
export const approvalDetailQueryOptions = (version: string, subjectId: string) =>
  scopedQuery(version, `approval:${subjectId}`, (signal) => fetchApprovalDetail(subjectId, signal));
