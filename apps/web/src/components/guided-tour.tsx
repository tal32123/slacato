...
function describeRunGate(
  ...
  if (detail.status === 'failed')
    return {
      waiting: false,
      notice:
        'This run failed, so it produced no brief for the next steps to read. Outside the spotlight, this page carries the "Run failed safely" notice above and the persisted timeline below, which record the phase it stopped in; Demo Diagnostics reports the dependency and model checks behind it. You can continue, or start another run from the deal first.'
    };
  return {
    waiting: false,
