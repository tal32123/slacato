...
const steps: readonly WalkthroughStep[] = [
  {
    ...
    assignmentRequirement:
      'Turn opportunity data and authorized evidence into an actionable, evidence-grounded brief.',
    persona: 'Maya Levin · Account Owner',
    screen: 'Settings → Deals → OPP-1001',
    action:
      'Select Maya in Settings, open OPP-1001, then choose Generate Brief. Open the run while it processes and return to the deal when it completes.',
    expectedProof:
      'The deal workspace shows structured strategy, risks, stakeholders, recommendations, and citations. The stable run URL shows retrieval, specialist, synthesis, and validation phases.',
    links: [
  ...
  },
  {
    ...
    assignmentRequirement:
      'Add reviewed synthetic Slack data, retrieve it with the supplied sources, and visibly cite it in the result.',
    persona: 'Maya Levin · Account Owner',
  ...
  },
  {
    ...
    assignmentRequirement:
      'Handle sensitive OPP-1003 recommendations through explicit, policy-derived human approvals.',
    persona: 'Nora Chen · Restricted Account Owner',
    screen: 'Settings → OPP-1003 → Run → Approvals',
    action:
      'Select Nora, generate the OPP-1003 brief, and follow the run into Awaiting approval. Then switch among Rina Vale, Tomas Reed, and Iris Wynn to inspect only the approval authority each persona holds.',
    expectedProof:
      'The run does not publish gated recommendations prematurely. Approval entries remain separate for Deal Desk, Sales Leader, Legal Reviewer, and any required account-owner confirmation.',
    links: [
  ...
  },
  {
    ...
    assignmentRequirement:
      'Demonstrate that an unauthorized user cannot access restricted OPP-1003 or learn its protected metadata.',
    persona: 'Harper Noor · Unauthorized Requester',
    screen: 'Settings → direct OPP-1003 URL',
    action:
      'Select Harper, then use the direct restricted-deal link below. Also check Deals and Runs while Harper is active.',
    expectedProof:
      'The direct request ends in an opaque authorization message. Protected deal name, account, brief, evidence, run, and approval details are absent; lists contain only Harper’s authorized scope.',
    links: [
  ...
  },
  {
    ...
    assignmentRequirement:
      'Make the agent workflow auditable and show real retrieval, persisted state, model configuration, and structured generation.',
    persona: 'Return to the persona that created the run',
    screen: 'Runs → run detail → Diagnostics',
    action:
      'Open a completed or active run and inspect its phase history, checkpoints, model attempts, evidence manifest, and validation status. Then open Diagnostics.',
    expectedProof:
      'Runs survive navigation at stable URLs. Diagnostics reports OpenRouter, the pinned GLM generation model, the explicit embedding model and index health, while the run provides the trace linking inputs to output.',
    links: [
    ...
    <div className="grid gap-8">
      <header className="grid gap-6 rounded-2xl border border-primary/25 bg-[linear-gradient(135deg,hsl(var(--primary)/0.11),hsl(var(--card))_60%)] p-6 sm:p-8 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-end">
        <div className="max-w-4xl">
          ...
          <h1 className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">
            See the assignment working, end to end
          </h1>
          <p className="mt-3 max-w-3xl leading-7 text-muted-foreground">
            Use the guided spotlight tour to move through the real interface, or keep this page open
            as a reference for the assignment scenarios and expected proof.
          </p>
          <Button
            className="mt-5 min-h-11"
            onClick={() => window.dispatchEvent(new Event('slacato:start-guided-tour'))}
          >
        ...
        <div className="rounded-xl border bg-card/85 p-4 shadow-sm">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {progressLabel}. Saved in this browser.
          </p>
          {completedCount > 0 && (
            <Button variant="ghost" size="sm" className="mt-2 px-0" onClick={reset}>
      ...
      <section aria-labelledby="map-heading">
        <div className="max-w-3xl">
          ...
          <h2 id="map-heading" className="mt-1 text-2xl font-semibold tracking-tight">
            Five scenarios tell the product story
          </h2>
          <p className="mt-2 leading-6 text-muted-foreground">
            The persona is part of each scenario because authorization changes what may be
            retrieved, generated, cited, approved, and even named.
          </p>
        </div>
          ...
          {steps.map((step, index) => {
            ...
            return (
              <li key={step.id}>
                <Card
                  className={cn(
                    'gap-0 overflow-hidden shadow-none transition-colors',
                  ...
                  <CardHeader className="border-b sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-start">
                    <span
                      className={cn(
                        ...
                        done ? 'border-primary bg-primary text-primary-foreground' : 'bg-secondary'
                      )}
                    >
                  ...
                  <CardContent className="grid gap-6 py-6 lg:grid-cols-3">
                    ...
                    <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 lg:col-span-3">
                      <div className="flex gap-3">
                        ...
                        />
                        <div>
                          <h3 className="text-sm font-semibold">Expected proof</h3>
                    ...
                    <div className="flex flex-wrap gap-2 lg:col-span-3">
                      {step.links.map((link) => (
                        <Button
                          key={`${step.id}:${link.to}:${link.label}`}
                          asChild
