    ...
    if (error instanceof SessionInvalidatedError) {
      try {
        ...
        if (session.authenticated) {
          const response = await queryClient.fetchQuery(runDetailQueryOptions(session.version, runId));
...
export function RunRoute(): React.JSX.Element {
  ...
  const query = useQuery({ ...runDetailQueryOptions(session.version, initial.runId), initialData: initial });
  ...
  useEffect(() => {
    ...
    const close = openRunEventStream({
      ...
      onEvent: (candidate) => {
        ...
        queryClient.setQueryData<RunDetailResponse>(queryKeys.scoped(session.version, `run:${detail.runId}`), (current) =>
          current === undefined ? current : applyRunEvent(current, candidate, generation, sessionRuntime.generation)
        );
  ...
  return (
    <article className="min-w-0" aria-labelledby="run-title">
      ...
      <p className="sr-only" aria-live="polite" aria-atomic="true">Run phase: {statusLabel(detail.status)}</p>
      <header className="mt-3 border-b pb-7">
        <div className="flex flex-wrap items-center gap-2">
          <StatusBadge status={detail.status === 'completed' ? 'ready' : ['failed', 'rejected', 'awaiting_approval'].includes(detail.status) ? 'attention' : 'readonly'} label={statusLabel(detail.status)} />
      ...
      <section className="border-b py-7" aria-labelledby="specialists-title">
        <h2 id="specialists-title" className="text-xl font-semibold">Specialists</h2>
        <ul className="mt-4 grid gap-3 sm:grid-cols-3">{detail.progress.specialists.map((specialist) => (
          <li key={specialist.name} className="rounded-lg border bg-card p-4"><p className="font-medium capitalize">{specialist.name}</p><p className="mt-1 text-sm capitalize text-muted-foreground">{specialist.status}</p></li>
        ))}</ul>
      </section>

      {detail.progress.completedSections.length > 0 && (
        <section className="border-b py-7" aria-labelledby="sections-title"><h2 id="sections-title" className="text-xl font-semibold">Completed validated sections</h2><ul className="mt-4 grid gap-2 sm:grid-cols-2">{detail.progress.completedSections.map((section) => <li key={section} className="flex items-center gap-2 text-sm"><CheckCircle2 aria-hidden="true" className="size-4 text-primary" />{section}</li>)}</ul></section>
      )}
