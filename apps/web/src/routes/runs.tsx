  ...
  return (
    <section data-tour="run-progress" aria-labelledby="runs-title">
      <header className="max-w-4xl">
        ...
        <h1 id="runs-title" className="mt-2 text-3xl font-semibold tracking-tight sm:text-4xl">
          Runs
        </h1>
        <p className="mt-3 leading-7 text-muted-foreground">
          Rejoin active work at its stable URL or inspect terminal outcomes. Every row is
          reauthorized for the current persona.
        </p>
      </header>
      {response.runs.length === 0 ? (
        <div className="mt-8 rounded-xl border border-dashed p-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Generate a brief from an authorized deal workspace to start one.
          </p>
        ...
        <ul className="mt-8 grid gap-3" aria-label="Authorized runs">
          {response.runs.map((run) => {
            ...
            return (
              <li
                key={run.runId}
                className="grid min-w-0 gap-4 rounded-xl border bg-card p-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-1 break-words text-sm text-muted-foreground">
                    {run.accountName} · Initiated by {run.initiatedBy}
                  </p>
                  <p className="mt-2 text-xs text-muted-foreground">
                    Updated <time dateTime={run.updatedAt}>{formatTime(run.updatedAt)}</time>
                  </p>
                </div>
                <Button
                  asChild
...
export function statusLabel(status: RunStatus): string {
  const labels: Record<RunStatus, string> = {
    ...
    specialists_running: 'Specialists running',
