...
export async function diagnosticsLoader({
  request
}: LoaderFunctionArgs): Promise<DemoDiagnosticsResponse | null> {
  try {
    ...
    if (error instanceof SessionInvalidatedError) {
      try {
        ...
        if (session.authenticated) {
          const diagnostics = await queryClient.fetchQuery(
            diagnosticsQueryOptions(session.version)
          );
    ...
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)] gap-8">
      <header className="max-w-4xl">
        ...
        <p className="mt-3 max-w-3xl leading-7 text-muted-foreground">
          SlaCato is a negotiation-preparation assistant. Its recommendations are internal,
          evidence-backed suggestions; sellers own judgment, and no control autonomously sends
          customer-facing content.
        </p>
        <Button asChild variant="link" className="mt-3 min-h-11 px-0">
          <Link to="/settings">
            <ArrowLeft aria-hidden="true" />
            Back to Persona &amp; session
          </Link>
        </Button>
      </header>
      ...
      <section data-tour="diagnostics" aria-labelledby="runtime-title">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 id="runtime-title" className="text-xl font-semibold">
              Runtime configuration
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Pinned, server-reported facts for this process. These controls cannot be changed here.
            </p>
          </div>
          <StatusBadge
            status={
              health.runtimeReadiness === 'ready'
                ? 'ready'
                : health.runtimeReadiness === 'unconfigured'
                  ? 'readonly'
                  : 'attention'
            }
            label={
        ...
        <Card className="shadow-none">
          <CardHeader className="border-b">
            <CardTitle className="flex items-center gap-2">
              <ServerCog aria-hidden="true" className="size-5 text-primary" />
          ...
          <CardContent>
            <dl className="grid gap-x-8 gap-y-5 sm:grid-cols-2 xl:grid-cols-3">
              ...
              <DiagnosticValue
                icon={Bot}
                label="Output mode"
                value={outputModeLabel(health.outputMode)}
              />
              <DiagnosticValue
                icon={Bot}
                label="Pinned generation model"
                value={health.pinnedGenerationModel}
              />
              <DiagnosticValue
                icon={SearchCheck}
                label="Pinned embedding model"
                value={health.pinnedEmbeddingModel}
              />
              <DiagnosticValue
                icon={Database}
                label="Index health"
                value={readinessLabel(health.indexHealth)}
              />
              <DiagnosticValue
                icon={ServerCog}
                ...
                {dependencyChecks.map(([name, status]) => (
                  <li key={name}>
                    <StatusBadge
                      ...
                      label={`${dependencyLabel(name)}: ${readinessLabel(status)}`}
                    />
                  </li>
                ))}
              </ul>
      ...
      <section aria-labelledby="permissions-title">
        <div className="mb-4 max-w-4xl">
          <h2 id="permissions-title" className="text-xl font-semibold">
            Canonical permission view
          </h2>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Source permissions for {session.persona.displayName} are shown independently from
            account-scoped approval authority. Request permission never grants decision authority.
          </p>
        </div>
        <PermissionMatrix
          grants={diagnostics.permissions}
          approvalAuthorities={diagnostics.approvalAuthorities}
        />
      </section>
    </div>
  );
...
function readinessLabel(
  status: ProviderHealthView['checks'][keyof ProviderHealthView['checks']]
): string {
