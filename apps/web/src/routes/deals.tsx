  ...
  return (
    <div className="grid gap-7">
      <header className="max-w-4xl">
        ...
        <h2 className="mt-3 text-base font-semibold">Welcome, {session.persona.displayName.split(/\s+/)[0] ?? session.persona.displayName}</h2>
        <p className="mt-2 max-w-3xl leading-7 text-muted-foreground">Every opportunity below is reauthorized at the database query boundary for {session.persona.role}. Open a brief-first workspace to review source-backed negotiation preparation and citations.</p>
      </header>
