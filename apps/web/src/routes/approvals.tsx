  ...
  return (
    <section data-tour="approvals" aria-labelledby="approvals-title">
      <header className="max-w-4xl">
        ...
        <p className="mt-3 leading-7 text-muted-foreground">Only categories your current persona can operate are shown. Every decision is bound to an immutable subject hash and current run version.</p>
      </header>
      <ApprovalSection title="Pending" entries={response.pending} empty="No approvals currently require your authority." pending />
      <ApprovalSection title="Decision history" entries={response.history} empty="No approval decisions are visible for this persona." />
    </section>
  );
}
...
function ApprovalSection({ title, entries, empty, pending = false }: Readonly<{
  ...
  return (
    <section className="mt-9" aria-labelledby={`approval-${pending ? 'pending' : 'history'}`}>
      <div className="flex items-center gap-2"><h2 id={`approval-${pending ? 'pending' : 'history'}`} className="text-xl font-semibold">{title}</h2><span className="rounded-full bg-muted px-2 py-0.5 text-xs font-medium">{entries.length}</span></div>
      {entries.length === 0 ? <div className="mt-4 rounded-xl border border-dashed p-6 text-center"><Inbox aria-hidden="true" className="mx-auto size-7 text-muted-foreground" /><p className="mt-3 text-sm text-muted-foreground">{empty}</p></div> : (
        <ul className="mt-4 grid gap-3">{entries.map((entry) => <ApprovalRow key={`${entry.approvalSubjectId}:${entry.entryId}`} entry={entry} pending={pending} />)}</ul>
      )}
    </section>
  );
}
  ...
  return (
    <li className="grid min-w-0 gap-4 rounded-xl border bg-card p-5 lg:grid-cols-[minmax(0,1.3fr)_minmax(14rem,0.8fr)_auto] lg:items-center">
      ...
      <div className="flex flex-col gap-2 sm:flex-row lg:flex-col">
        <Button asChild><Link to={`/approvals/${encodeURIComponent(entry.approvalSubjectId)}`}>{pending ? 'Review' : 'View decision'}<Scale aria-hidden="true" /></Link></Button>
        <Button asChild variant="outline"><Link to={`/runs/${encodeURIComponent(entry.runId)}`}>View run</Link></Button>
      </div>
    </li>
  );
}
