        ...
        <div className="mt-5 grid gap-5">
          <SubjectSection title="Deal snapshot">
            <dl className="grid gap-3 text-sm sm:grid-cols-2">
              ...
              {detail.payload.dealSnapshot.owner && (
                <BriefFact label="Owner" value={detail.payload.dealSnapshot.owner} />
              )}
              {detail.payload.dealSnapshot.closeDate && (
                <BriefFact label="Close date" value={detail.payload.dealSnapshot.closeDate} />
              )}
              {detail.payload.dealSnapshot.amount !== undefined && (
                <BriefFact
                  label="Amount"
                  value={`${detail.payload.dealSnapshot.currency ?? ''} ${detail.payload.dealSnapshot.amount.toLocaleString()}`.trim()}
                />
              )}
            </dl>
          </SubjectSection>
          <SubjectSection title="Executive summary">
            <p className="leading-7">{detail.payload.executiveSummary.narrative}</p>
          </SubjectSection>
          <SubjectSection title="Buyer goals and business drivers">
            <BriefList
              title="Goals"
              values={detail.payload.buyerGoalsAndBusinessDrivers.goals}
              empty="No buyer goals were supported by the validated evidence."
            />
            <BriefList
              title="Business drivers"
              values={detail.payload.buyerGoalsAndBusinessDrivers.businessDrivers}
              empty="No business drivers were supported by the validated evidence."
            />
          </SubjectSection>
          <SubjectSection title="Stakeholder map">
            {detail.payload.stakeholderMap.stakeholders.length === 0 ? (
              <EmptyState>No stakeholders were supported by the validated evidence.</EmptyState>
            ) : (
              <ul className="grid gap-3">
                {detail.payload.stakeholderMap.stakeholders.map((stakeholder) => (
                  <li
                    key={`${stakeholder.name}:${stakeholder.role}`}
            ...
            <BriefList
              title="Coverage gaps"
              values={detail.payload.stakeholderMap.coverageGaps ?? []}
              empty="No stakeholder coverage gaps were recorded."
            />
          </SubjectSection>
          <SubjectSection title="Negotiation state">
            <p className="leading-7">{detail.payload.negotiationState.currentState}</p>
            <BriefList
              title="Leverage"
              values={detail.payload.negotiationState.leverage ?? []}
              empty="No leverage was supported by the validated evidence."
            />
            <BriefList
              title="Risks"
              values={detail.payload.negotiationState.risks}
              empty="No negotiation risks were recorded."
            />
          </SubjectSection>
          <SubjectSection title="Recommended next actions">
            {detail.payload.recommendedNextActions.actions.length === 0 ? (
              <EmptyState>No next actions were proposed.</EmptyState>
            ) : (
              <ul className="grid gap-3">
                {detail.payload.recommendedNextActions.actions.map((action) => (
          ...
          <SubjectSection title="Missing information">
            {detail.payload.missingInformation.items.length === 0 ? (
              <EmptyState>No material information gaps were recorded.</EmptyState>
            ) : (
              <ul className="grid gap-3">
                {detail.payload.missingInformation.items.map((item) => (
          ...
          <SubjectSection title="Authorized evidence summaries">
            {detail.payload.sourceEvidence.evidence.length === 0 ? (
              <EmptyState>No evidence summaries were included.</EmptyState>
            ) : (
              <ul className="grid gap-3">
                {detail.payload.sourceEvidence.evidence.map((evidence) => (
                  <li key={evidence.evidenceId} className="rounded-lg border p-3">
                    ...
                    {evidenceIds.has(evidence.evidenceId) && (
                      <Button asChild variant="link" className="h-auto min-h-11 px-0">
                        <Link
                          to={`/deals/${encodeURIComponent(detail.opportunityId)}?evidence=${encodeURIComponent(evidence.evidenceId)}`}
                        >
          ...
          <SubjectSection title="Confidence and review warnings">
            <p className="text-2xl font-semibold">
              {Math.round(detail.payload.confidenceAndReviewWarnings.overallConfidence * 100)}%{' '}
              <span className="text-sm font-normal text-muted-foreground">overall confidence</span>
            </p>
            {detail.payload.confidenceAndReviewWarnings.warnings.length === 0 ? (
              <EmptyState>No review warnings were recorded.</EmptyState>
            ) : (
              <ul className="mt-3 grid gap-3">
                {detail.payload.confidenceAndReviewWarnings.warnings.map((warning) => (
                  <li
                    key={warning.code}
        ...
        >
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            <Button
              ...
              onClick={() => submit('approve_unchanged')}
            >
              <CheckCircle2 aria-hidden="true" />
              {activeAction === 'approve_unchanged' ? 'Recording approval…' : 'Approve unchanged'}
            </Button>
            <Button
              type="button"
              variant="outline"
          ...
          {mode !== null && (
            <div
              id={mode === 'edit' ? 'edit-decision-fields' : 'reject-decision-fields'}
            ...
            >
                ...
                <>
                  <div className="grid gap-2">
                    ...
                    <Textarea
                      ...
                      aria-invalid={fieldErrors.executiveSummary ? true : undefined}
                      aria-describedby={
                    ...
                    <p id="executive-summary-help" className="text-xs text-muted-foreground">
                      {executiveSummary.length} of 8,000 characters
                    </p>
                    {fieldErrors.executiveSummary && (
                  ...
                  <div className="grid gap-2">
                    ...
                    <Textarea
                      ref={negotiationStateRef}
                      id="negotiation-state"
                      maxLength={8000}
                      value={negotiationState}
                      aria-invalid={fieldErrors.negotiationState ? true : undefined}
                      aria-describedby={
                        fieldErrors.negotiationState
                          ? 'negotiation-state-error negotiation-state-help'
                          : 'negotiation-state-help'
                      }
                      onChange={(event) => setNegotiationState(event.target.value)}
                    />
                    <p id="negotiation-state-help" className="text-xs text-muted-foreground">
                      {negotiationState.length} of 8,000 characters
                    </p>
                    {fieldErrors.negotiationState && (
                  ...
                  <div className="grid max-w-xs gap-2">
                    ...
                    <Input
                      ...
                      value={confidence}
                      aria-invalid={
                        fieldErrors.confidence || fieldErrors.editedPayload ? true : undefined
                      }
                      aria-describedby={
                        fieldErrors.confidence
                          ? 'confidence-error'
                          : fieldErrors.editedPayload
              ...
              <div className="grid gap-2">
                ...
                <p id="decision-rationale-help" className="text-xs text-muted-foreground">
                  {rationale.length} of 4,000 characters
                </p>
                {fieldErrors.rationale && (
              ...
              <Button
                type="button"
                disabled={mutation.isPending}
                onClick={() => submit(mode === 'edit' ? 'edit_and_approve' : 'reject')}
              >
        ...
        {detail.decisions.length === 0 ? (
          ...
          <ol className="mt-4 grid gap-3">
            {detail.decisions.map((decision) => (
              <li
                key={`${decision.decidedAt}:${decision.actorName}`}
                className="rounded-lg border p-4"
              >
                <div className="flex flex-wrap items-center gap-2">
                  <StatusBadge
                    status={decision.action === 'reject' ? 'attention' : 'ready'}
                    label={label(decision.action)}
                  />
                  <span className="text-sm font-medium">{decision.actorName}</span>
                </div>
                <p className="mt-2 text-sm text-muted-foreground">
                  {label(decision.authority)} ·{' '}
                  <time dateTime={decision.decidedAt}>{formatTime(decision.decidedAt)}</time>
                  {decision.changed ? ' · Subject changed' : ''}
                </p>
                {decision.rationale && (
                  <p className="mt-3 text-sm leading-6">{decision.rationale}</p>
                )}
                {decision.diff && (
                  <div className="mt-4 rounded-lg bg-muted/50 p-4">
                    <p className="text-sm font-medium">Recorded changes</p>
                    {decision.diff.fields.length > 0 && (
                      <dl className="mt-2 grid gap-3">
                        {decision.diff.fields.map((field) => (
                          <div key={field.field}>
                    ...
                    <p className="mt-3 text-xs text-muted-foreground">
                      Sections changed:{' '}
                      {decision.diff.changedSections.map(label).join(', ') || 'None'}
                    </p>
                  </div>
                )}
              </li>
