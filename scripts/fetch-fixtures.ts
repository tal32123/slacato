...
export type FixtureAttribution = Readonly<{
  repository: string; commit: string; sourceCommittedAt: string;
...
export function fetchCanonicalFixtures(options: FixtureFetchOptions): FixtureAttribution {
  if (!/^[0-9a-f]{40}$/.test(options.commit)) throw new Error('Pinned fixture revision must be a lowercase 40-character SHA-1');
  ...
  try {
    ...
    if (resolvedCommit !== options.commit) throw new Error(`Fixture revision mismatch: expected ${options.commit}, received ${resolvedCommit}`);
    const sourceCommittedAt = git(temporaryRoot, ['show', '-s', '--format=%cI', 'FETCH_HEAD']).trim();
    const files = options.files.map((path) => {
      if (path.startsWith('/') || path.includes('..') || path.includes('\\')) throw new Error(`Unsafe fixture path: ${path}`);
      const content = execFileSync('git', ['show', `FETCH_HEAD:synthetic_data/${path}`], { cwd: temporaryRoot, maxBuffer: 8 * 1024 * 1024 });
    ...
    writeFileSync(join(options.destination, 'source-attribution.json'), `${JSON.stringify(attribution, null, 2)}\n`);
    return attribution;
  } finally {
...
function main(): void {
  ...
  process.stdout.write(`Verified ${result.commit}; copied ${result.files.length} canonical files.\n`);
}
