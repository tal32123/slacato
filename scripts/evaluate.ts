...
export function evaluateRetrievalResults(inputs: readonly RetrievalEvaluationInput[]) {
  ...
  const qualityCases = cases.filter((_, index) => {
    const input = inputs[index];
    if (input === undefined) {
      throw new Error('Evaluation case cardinality does not match its inputs');
    }
    return !input.denied;
  });
...
const unusedGenerationLedger: ProviderAttemptLedger = {
  async beginAttempt() {
    throw new Error('Generation is unavailable during deterministic retrieval evaluation');
  },
  async settleAttempt() {},
...
async function runRetrievalEvaluation(): Promise<ReturnType<typeof evaluateRetrievalResults>> {
  const golden = goldenSchema.parse(
    JSON.parse(await readFile(resolve('evals/golden-retrieval.json'), 'utf8'))
  );
  ...
  try {
    ...
    await ingestFixtureRecords({
      ...
      databaseUrl: evaluationUrl.toString()
    });
    ...
    for (const testCase of golden.cases) {
      const opportunity = await database.sql<
        { restricted: boolean }[]
      >`select restricted from opportunities where id = ${testCase.opportunityId} and account_id = ${testCase.accountId}`;
      ...
      const denied =
        grants.length === 0 ||
        (opportunity[0]?.restricted === true && !grants.some((grant) => grant.can_read_restricted));
      const sourceTypes = denied
        ? [...AUTHORIZED_SOURCE_TYPES]
        : [
            ...new Set(
              grants
                .map((grant) => grant.source_type)
                .filter((source): source is AuthorizedSourceType =>
                  AUTHORIZED_SOURCE_TYPES.includes(source as AuthorizedSourceType)
                )
            )
          ].sort();
...
async function main(): Promise<void> {
  if (process.argv[2] !== 'retrieval')
    throw new Error('Usage: pnpm tsx scripts/evaluate.ts retrieval');
  const report = await runRetrievalEvaluation();
  await mkdir(resolve('evals/reports'), { recursive: true });
  await writeFile(
    resolve('evals/reports/retrieval.json'),
