...
export function createConfiguredModelGateways(
  ...
  const runScopedEmbedding =
    (
      ...
      attemptLedger: PostgresProviderAttemptLedger,
      provider: ConfiguredModelGateways['provider'],
      model: string
    ) =>
  ...
  if (environment.AI_PROVIDER === 'openrouter') {
    if ('mock' in options && options.mock !== undefined)
      throw new Error('OpenRouter composition does not accept a mock fixture resolver');
    const attemptLedger = options.attemptLedger;
  ...
  if ('mock' in options && options.mock !== undefined)
    throw new Error('Ollama composition does not accept a mock fixture resolver');
  const attemptLedger = options.attemptLedger;
