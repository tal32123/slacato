import { runEventToPublishSchema, traceSpanSchema } from '@slacato/contracts';
import {
  ...
  type ApprovalDecisionStoreResult,
  ...
  type ReplaceApprovalSubjectInput,
...
type RunRow = Readonly<{
  ...
  start_request_hash?: string | undefined;
}>;
...
function asRun(row: RunRow): WorkflowRun {
  return {
    id: row.id as WorkflowRun['id'],
    opportunityId: row.opportunity_id as WorkflowRun['opportunityId'],
    requestedBy: row.requested_by as WorkflowRun['requestedBy'],
    status: row.status,
    version: row.version,
...
function asLease(row: InvocationRow): StepLease {
  return {
    invocationId: row.id,
    causalCommandId: row.causal_command_id,
    runId: row.run_id as StepLease['runId'],
    step: row.step,
    owner: row.owner,
...
async function runById(sql: SqlExecutor, id: string, lock = false): Promise<RunRow | undefined> {
  const rows = lock
    ? await sql<
        RunRow[]
      >`select id, opportunity_id, requested_by, status, version, generation_provider, generation_model, start_request_hash from runs where id = ${id} for update`
    : await sql<
        RunRow[]
      >`select id, opportunity_id, requested_by, status, version, generation_provider, generation_model, start_request_hash from runs where id = ${id}`;
  return rows[0];
}
...
async function insertCommand(sql: SqlExecutor, command: WorkflowCommand): Promise<void> {
  ...
  >`select id, run_id, type, payload, idempotency_key from outbox_commands where id = ${command.id} or idempotency_key = ${command.idempotencyKey} for update`;
  if (existing.length > 0) {
    ...
    if (
      ...
      throw new DomainConflictError('Outbox idempotency key conflicts with another command');
    return;
  }
...
/** Resolves a persistence-neutral trace parent reference against spans already stored for the run. */
async function resolveTraceParent(
  sql: SqlExecutor,
  runId: string,
  parent: TraceParentReference | undefined
): Promise<string | undefined> {
  if (parent === undefined) return undefined;
  if (parent.type === 'direct') return parent.spanId;
  if (parent.type === 'if_present') {
    return (
      await sql<
        { span_id: string }[]
      >`select span_id from trace_spans where span_id = ${parent.spanId}`
    )[0]?.span_id;
  }
  for (const candidate of parent.candidates) {
    const spanId =
      candidate.step === undefined
        ? (
            await sql<{ span_id: string }[]>`select span_id from trace_spans
          where run_id = ${runId} and kind in ${sql(candidate.kinds)}
          order by started_at desc, span_id desc limit 1`
          )[0]?.span_id
        : (
            await sql<{ span_id: string }[]>`select span_id from trace_spans
          where run_id = ${runId} and kind in ${sql(candidate.kinds)} and step = ${candidate.step}
          order by started_at desc, span_id desc limit 1`
          )[0]?.span_id;
    if (spanId !== undefined) return spanId;
  }
  return undefined;
}

/** Persists one projected trace span idempotently after resolving its parent inside the current transaction. */
async function appendProjectedTrace(
  sql: SqlExecutor,
  projected: ProjectedTraceSpan
): Promise<void> {
  const now = new Date().toISOString();
  const parentSpanId = await resolveTraceParent(sql, projected.runId, projected.parent);
  const span = traceSpanSchema.parse({
    traceId: projected.traceId,
    spanId: projected.spanId,
  ...
  const existing = (
    ...
    from trace_spans where id = ${span.spanId} for update`
  )[0];
  if (existing !== undefined) {
    ...
    if (canonicalJson(persisted) !== canonicalJson(proposed)) {
      throw new DomainConflictError(
        `Trace span ID conflicts with different content: ${span.spanId}`
      );
    }
    return;
  }
...
export async function persistTraceProjection(
  ...
  const attempt = projection.generationAttempt;
  if (attempt !== undefined) {
    await sql`insert into generation_attempts
      ...
      values (${attempt.id}, ${attempt.runId}, ${attempt.invocationId}, ${attempt.logicalGenerationId}, ${attempt.operation}, ${attempt.ordinal},
        ${attempt.status}, ${attempt.provider}, ${attempt.model}, ${attempt.outputMode}, ${attempt.validationAttempts},
        ${attempt.possibleDuplicate}, ${attempt.inputTokens}, ${attempt.outputTokens}, now())`;
  }
  for (const span of projection.spans) await appendProjectedTrace(sql, span);
}

/** Loads durable model attempts and maps storage column names to the projector's typed boundary. */
...
/** Projects and persists generation trace spans from a saved checkpoint and its durable attempts. */
async function appendGenerationTrace(sql: SqlExecutor, input: SaveCheckpointInput): Promise<void> {
  const attempts =
    input.logicalGenerationId === undefined
      ? []
      : await generationAttempts(sql, input.runId, input.logicalGenerationId);
...
async function appendEvent(
  ...
  const terminalPayload = ['complete', 'fail', 'approval_rejected', 'cancel'].includes(type)
...
async function replayDecision(
    ...
    await sql<
      {
        ...
        result_run_version: number;
        result_status: 'awaiting_approval' | 'finalizing' | 'rejected';
        result_quorum_satisfied: boolean;
  ...
  if (decision.request_hash !== input.requestHash)
    throw new DomainConflictError('Decision idempotency key conflicts with another decision');
  ...
  const approvalSubjectId =
    decision.action === 'edit_and_approve'
      ? decision.superseded_by_subject_id
      : decision.approval_subject_id;
  if (approvalSubjectId === null)
    throw new DomainConflictError('Replacement approval subject is unavailable');
  return {
    run: {
      ...asRun(currentRun),
      status: decision.result_status,
      version: decision.result_run_version
    },
    approvalSubjectId,
...
async function ownLease(
  sql: SqlExecutor,
  input: Readonly<{ invocationId: string; invocationOwner: string; leaseToken: string }>
): Promise<void> {
  const rows = await sql<
    { id: string }[]
  >`select id from step_invocations where id = ${input.invocationId} and owner = ${input.invocationOwner} and lease_token = ${input.leaseToken} and status = 'leased' and lease_expires_at > now() for update`;
  if (rows.length !== 1)
    throw new DomainConflictError('Step lease is no longer owned by this worker');
}
...
async function completeLease(
  ...
  input: Readonly<{
    ...
    leaseToken: string;
    causalCommandId?: string | undefined;
  }>
): Promise<string> {
  const rows = await sql<
    { causal_command_id: string }[]
  >`update step_invocations set status = 'completed', completed_at = now()
  ...
  if (
    commandId === undefined ||
    (input.causalCommandId !== undefined && commandId !== input.causalCommandId)
  )
    throw new DomainConflictError('Step lease is no longer owned by this worker');
  const consumed =
    await sql`update outbox_commands set consumed_at = now() where id = ${commandId} and run_id = ${input.runId} and status = 'published' and consumed_at is null`;
...
async function storeCheckpoint(
  ...
  const rows = await sql<
    { payload: Record<string, unknown>; logical_generation_id: string | null }[]
  >`select payload, logical_generation_id from workflow_checkpoints where run_id = ${input.runId} and step = ${input.step} for update`;
  const existing = rows[0];
  if (existing !== undefined) {
    if (
      canonicalJson(existing.payload) !== canonicalJson(input.checkpoint) ||
      (existing.logical_generation_id ?? undefined) !== input.logicalGenerationId
    )
      throw new DomainConflictError('Checkpoint replay conflicts with persisted content');
    return;
  }
...
async function persistGeneratedArtifact(
  ...
  const existing = await sql<
    { content_hash: string; outcome: string; generation_metadata: Record<string, unknown> }[]
  >`select content_hash, outcome, generation_metadata from specialist_artifacts where logical_generation_id = ${input.logicalGenerationId} for update`;
  const contentHash = hashApprovalPayload(value);
  if (existing[0] !== undefined) {
    if (
      existing[0].content_hash !== contentHash ||
      existing[0].outcome !== status ||
      canonicalJson(existing[0].generation_metadata) !==
        canonicalJson(input.checkpoint.generation ?? {})
    )
...
export class PostgresWorkflowStore implements WorkflowStore {
  ...
  public async findRunByIdempotencyKey(
    input: Readonly<{
      ...
      requestedBy: WorkflowRun['requestedBy'];
    ...
    const row = (
      ...
      from runs where idempotency_key = ${input.idempotencyKey} and requested_by = ${input.requestedBy} and opportunity_id = ${input.opportunityId}`
    )[0];
  ...
  public async getRun(runId: WorkflowRun['id']): Promise<WorkflowRun | undefined> {
    const row = await runById(this.database.sql, runId);
    return row === undefined ? undefined : asRun(row);
  }
  ...
  public async cancelRun(input: Parameters<WorkflowStore['cancelRun']>[0]): Promise<WorkflowRun> {
    return this.database.sql.begin(async (sql) => {
      ...
      if (current.version !== input.expectedVersion)
        throw new DomainConflictError('Run version is stale');
      ...
      const row = (
        await sql<
          RunRow[]
        >`update runs set status = ${nextStatus}, version = version + 1, updated_at = now()
      ...
      await appendEvent(sql, input.runId, 'cancel', {
        version: row.version,
  ...
  /** Creates a workflow run and its initial authorization trace atomically with the first command. */
  public async startRun(input: StartRunInput): Promise<WorkflowRun> {
    ...
    return this.database.sql.begin(async (sql) => {
      ...
      const replay = (
        ...
        where idempotency_key = ${input.idempotencyKey} and requested_by = ${input.requestedBy} and opportunity_id = ${input.opportunityId} for update`
      )[0];
      if (replay !== undefined) {
      ...
      const rows = await sql<
        RunRow[]
      >`insert into runs (id, opportunity_id, requested_by, status, generation_provider, generation_model, idempotency_key, start_request_hash, version)
        ...
        returning id, opportunity_id, requested_by, status, version, generation_provider, generation_model, start_request_hash`;
      const row = rows[0];
      if (row === undefined) throw new DomainConflictError('Unable to create run');
      await sql`insert into run_budgets (run_id, max_calls, deadline_ms, deadline_at) values
        (${input.id}, ${input.budget.maxCalls}, ${input.budget.deadlineMs}, now() + (${input.budget.deadlineMs}::text || ' milliseconds')::interval)`;
      await persistTraceProjection(
        sql,
        projectWorkflowTrace({
          type: 'run_started',
      ...
      await appendEvent(sql, input.id, 'run_created', {
        status: input.status,
  ...
  public async claimStep(
    ...
    return this.database.sql.begin(async (sql) => {
      ...
      const active = (
        await sql<
          InvocationRow[]
        >`select id, run_id, step, owner, lease_token, causal_command_id, lease_expires_at, attempt from step_invocations where run_id = ${input.runId} and step = ${input.step} and status = 'leased' order by attempt desc limit 1 for update`
      )[0];
      if (active !== undefined && new Date(active.lease_expires_at) > now) return undefined;
      if (active !== undefined)
        await sql`update step_invocations set status = 'abandoned', completed_at = ${now.toISOString()}::timestamptz where id = ${active.id}`;
      const attempt = (
        await sql<
          { attempt: number }[]
        >`select coalesce(max(attempt), 0) + 1 as attempt from step_invocations where run_id = ${input.runId} and step = ${input.step}`
      )[0]?.attempt;
      ...
      const row = (
        await sql<
          InvocationRow[]
        >`insert into step_invocations (id, run_id, step, owner, lease_token, causal_command_id, lease_expires_at, heartbeat_at, attempt)
  ...
  public async heartbeatStep(
    ...
    const rows = await this.database.sql<
      InvocationRow[]
    >`update step_invocations set heartbeat_at = ${now.toISOString()}::timestamptz, lease_expires_at = ${expires.toISOString()}::timestamptz
  ...
  public async abandonStep(input: Parameters<WorkflowStore['abandonStep']>[0]): Promise<void> {
    const released = await this.database
      .sql`update step_invocations set status = 'abandoned', completed_at = now()
  ...
  public async getCheckpoint(
    input: Readonly<{ runId: WorkflowRun['id']; step: string }>
  ): Promise<Readonly<Record<string, unknown>> | undefined> {
    return (
      await this.database.sql<
        { payload: Record<string, unknown> }[]
      >`select payload from workflow_checkpoints where run_id = ${input.runId} and step = ${input.step}`
    )[0]?.payload;
  }
  ...
  /** Commits a workflow step, its trace projection, and the next command in one transaction. */
  public async commitStepAndEnqueueNext(input: CommitStepInput): Promise<WorkflowRun> {
    if (input.nextCommand.runId !== input.runId)
      throw new DomainConflictError('Outbox command run does not match workflow run');
    return this.database.sql.begin(async (sql) => {
      const current = await runById(sql, input.runId, true);
      if (current === undefined) throw new DomainNotFoundError('run');
      if (current.version !== input.expectedVersion)
      ...
      await storeCheckpoint(sql, {
        runId: input.runId,
        step: input.checkpointStep ?? input.event,
        invocationId: input.invocationId,
        checkpoint: input.checkpoint
      });
      if (input.artifact !== undefined) {
        const contentHash =
          input.artifact.contentHash ?? hashApprovalPayload(input.artifact.content);
      ...
      if (input.event === 'retrieval_completed') {
        await persistTraceProjection(
          sql,
          projectWorkflowTrace({
      ...
      await appendEvent(sql, input.runId, input.event, {
        version: row.version,
        status: nextStatus
      });
  ...
  /** Moves a run to approval while storing its subject, requirements, and trace projection atomically. */
  public async awaitApproval(input: AwaitApprovalInput): Promise<WorkflowRun> {
    if (hashApprovalPayload(input.subject.payload) !== input.subject.subjectHash)
      throw new DomainConflictError('Approval subject hash does not match its payload');
    return this.database.sql.begin(async (sql) => {
      const current = await runById(sql, input.runId, true);
      if (current === undefined) throw new DomainNotFoundError('run');
      if (current.version !== input.expectedVersion)
      ...
      for (const [ordinal, entry] of input.subject.entries.entries())
        await sql`insert into approval_requirement_entries (id, approval_subject_id, category, eligible_authorities, policy_triggers, depends_on, ordinal)
      ...
      await appendEvent(sql, input.runId, 'awaiting_approval', {
        version: row.version,
        subjectHash: input.subject.subjectHash,
        quorumVersion: input.subject.quorumVersion
      });
      return asRun(row);
    });
  }
  ...
  public async getApprovalSubject(
    input: Readonly<{ runId: WorkflowRun['id']; approvalSubjectId?: string | undefined }>
    ...
    const subjects =
      input.approvalSubjectId === undefined
        ? await this.database.sql<
            SubjectRow[]
          >`select id, run_id, draft_version, subject_hash, payload, section_ids, recommendation_ids, citation_ids, policy_triggers, quorum_version, superseded_by_subject_id from approval_subjects where run_id = ${input.runId} and superseded_by_subject_id is null order by draft_version desc limit 1`
        : await this.database.sql<
            SubjectRow[]
          >`select id, run_id, draft_version, subject_hash, payload, section_ids, recommendation_ids, citation_ids, policy_triggers, quorum_version, superseded_by_subject_id from approval_subjects where run_id = ${input.runId} and id = ${input.approvalSubjectId}`;
    const subject = subjects[0];
    if (subject === undefined) return undefined;
    const entries = await this.database.sql<
      {
        id: string;
        category: ApprovalCategory;
        eligible_authorities: ApprovalAuthority[];
        policy_triggers: string[];
        depends_on: string[];
      }[]
    >`select id, category, eligible_authorities, policy_triggers, depends_on from approval_requirement_entries where approval_subject_id = ${subject.id} order by ordinal`;
    ...
    >`select action, entry_id, category, authority, actor_id, original_payload, approved_payload, approved_subject_hash, edited_payload, diff, rationale, request_hash, created_at from approval_decisions where approval_subject_id = ${subject.id} order by created_at, id`;
    return {
      id: subject.id,
      runId: subject.run_id as WorkflowRun['id'],
      draftVersion: subject.draft_version,
      subjectHash: subject.subject_hash,
      payload: dealBriefSchema.parse(subject.payload),
      sectionIds: subject.section_ids,
      recommendationIds: subject.recommendation_ids,
      citationIds: subject.citation_ids,
      policyTriggers: subject.policy_triggers,
      quorumVersion: subject.quorum_version,
      entries: entries.map(
        (entry): ApprovalRequirementEntry => ({
          id: entry.id,
          category: entry.category,
          eligibleAuthorities: entry.eligible_authorities,
          policyTriggers: entry.policy_triggers,
      ...
      decisions: decisions.map(
        (decision): ApprovalDecision => ({
          ...
          actorId: decision.actor_id as ApprovalDecision['actorId'],
          originalPayload: dealBriefSchema.parse(decision.original_payload),
          approvedPayload: dealBriefSchema.parse(decision.approved_payload),
          approvedSubjectHash: decision.approved_subject_hash,
          ...(decision.edited_payload === null
            ? {}
            : { editedPayload: dealBriefSchema.parse(decision.edited_payload) }),
          ...(decision.diff === null ? {} : { diff: decision.diff }),
          ...(decision.rationale === null ? {} : { rationale: decision.rationale }),
          requestHash: decision.request_hash,
          decidedAt: new Date(decision.created_at).toISOString()
        })
      ),
      ...(subject.superseded_by_subject_id === null
        ? {}
  ...
  public async findDecisionByIdempotencyKey(
    input: Parameters<WorkflowStore['findDecisionByIdempotencyKey']>[0]
  ) {
    return replayDecision(this.database.sql, input);
  }
  /** Records an approval decision and its trace before conditionally scheduling finalization. */
  public async recordDecisionAndEnqueueFinalization(
    ...
    return this.database.sql.begin(async (sql) => {
      ...
      const prior = (
        await sql<
          { request_hash: string }[]
        >`select request_hash from approval_decisions where idempotency_key = ${input.idempotencyKey} for update`
      )[0];
      if (prior !== undefined) {
        ...
        if (replay === undefined)
          throw new DomainConflictError('Persisted decision result is unavailable');
        return {
          run: replay.run,
      ...
      const subject = (
        await sql<
          { subject_hash: string }[]
        >`select subject_hash from approval_subjects where id = ${input.approvalSubjectId} and run_id = ${input.runId} for share`
      )[0];
      if (subject?.subject_hash !== input.expectedSubjectHash)
      ...
      const entry = (
        ...
        >`select category, eligible_authorities, depends_on,
        (select count(*)::int from approval_requirement_entries where approval_subject_id = ${input.approvalSubjectId}) required
        from approval_requirement_entries where approval_subject_id = ${input.approvalSubjectId} and id = ${input.entryId} for share`
      )[0];
      if (
        entry === undefined ||
        entry.category !== input.category ||
        !entry.eligible_authorities.includes(input.authority)
      )
        throw new DomainConflictError('Approval category or authority mismatch');
      ...
      const approvedIds = new Set(approved.map(({ entry_id }) => entry_id));
      if (entry.depends_on.some((dependency) => !approvedIds.has(dependency)))
        throw new DomainConflictError('Approval dependencies are incomplete');
      if (
        approved.some(
          ({ approved_subject_hash }) =>
            approved_subject_hash !== input.decision.approvedSubjectHash
        )
      )
        throw new DomainConflictError('Approval decisions do not bind the same effective snapshot');
      const oppositeCommercialAuthority =
        input.authority === 'deal_desk'
          ...
          : input.authority === 'sales_leader'
            ...
            : undefined;
      if (
        input.category === 'commercial_discount' &&
        oppositeCommercialAuthority !== undefined &&
        approved.some(
          (prior) =>
            prior.category === 'commercial_discount' &&
            prior.authority === oppositeCommercialAuthority &&
      ...
      await appendEvent(sql, input.runId, event ?? 'approval_entry_recorded', {
        version: row.version,
        approvalSubjectId: input.approvalSubjectId,
        entryId: input.entryId,
        category: input.category,
        ...
        approvedSubjectHash: input.decision.approvedSubjectHash
      });
  ...
  /** Replaces an approval subject and projects the edited decision and new requirements atomically. */
  public async replaceApprovalSubject(
    ...
    return this.database.sql.begin(async (sql) => {
      ...
      if (prior !== undefined) {
        ...
        if (replay === undefined)
          throw new DomainConflictError('Persisted decision result is unavailable');
        const replaySubject = await this.getApprovalSubject({
          runId: input.runId,
      ...
      const old = (
        await sql<
          { id: string }[]
        >`select id from approval_subjects where id = ${input.priorSubjectId} and run_id = ${input.runId} and superseded_by_subject_id is null for update`
      )[0];
      ...
      const row = (
        await sql<
          RunRow[]
        >`update runs set version = version + 1, updated_at = now() where id = ${input.runId} and version = ${input.expectedVersion}
        returning id, opportunity_id, requested_by, status, version, generation_provider, generation_model, start_request_hash`
      )[0];
      if (row === undefined) throw new DomainConflictError('Run version is stale');
      await persistTraceProjection(
        ...
        projectWorkflowTrace({
          ...
          runId: input.runId,
          version: row.version,
          priorSubjectId: input.priorSubjectId,
          priorDecision: {
      ...
      await appendEvent(sql, input.runId, 'approval_subject_replaced', {
        priorSubjectId: input.priorSubjectId,
        approvalSubjectId: input.subject.id,
        subjectHash: input.subject.subjectHash
      });
      return {
        run: asRun(row),
        subject: { ...input.subject, draftVersion: row.version, decisions: [] },
        replayed: false
      };
    });
  }
  ...
  public async findRegenerationByIdempotencyKey(
    input: Parameters<WorkflowStore['findRegenerationByIdempotencyKey']>[0]
  ): Promise<WorkflowRun | undefined> {
    const idempotencyHash = hashApprovalPayload(input.idempotencyKey);
    const event = (
      await this.database.sql<
        { run_id: string; request_hash: string }[]
      >`select run_id, payload->>'requestHash' request_hash
      from run_events where type = 'regeneration_requested' and payload->>'idempotencyHash' = ${idempotencyHash}
      order by created_at limit 1`
    )[0];
    if (event === undefined) return undefined;
    if (event.request_hash !== hashApprovalPayload(input.requestHash))
      throw new DomainConflictError('Regeneration idempotency key conflicts with another command');
  ...
  public async regenerateRun(input: RegenerateRunInput): Promise<WorkflowRun> {
    return this.database.sql.begin(async (sql) => {
      ...
      if (replay !== undefined) {
        if (replay.request_hash !== hashApprovalPayload(input.requestHash))
          throw new DomainConflictError(
            'Regeneration idempotency key conflicts with another command'
          );
        const replayRun = await runById(sql, replay.run_id, true);
        if (replayRun === undefined) throw new DomainNotFoundError('run');
      ...
      if (
        ...
        !['awaiting_approval', 'rejected'].includes(current.status)
      )
        throw new DomainConflictError('Run cannot be regenerated');
      const row = (
        await sql<
          RunRow[]
        >`update runs set status = 'synthesizing', version = version + 1, updated_at = now() where id = ${input.runId} and version = ${input.expectedVersion}
        returning id, opportunity_id, requested_by, status, version, generation_provider, generation_model, start_request_hash`
      )[0];
      if (row === undefined) throw new DomainConflictError('Run version is stale');
      await insertCommand(sql, input.command);
      await appendEvent(sql, input.runId, 'regeneration_requested', {
        idempotencyHash,
        requestHash: hashApprovalPayload(input.requestHash),
  ...
  /** Finalizes the approved brief and records its completion trace in the same transaction. */
  public async finalizeRun(input: FinalizeRunInput): Promise<WorkflowRun> {
    if (hashApprovalPayload(input.payload) !== input.subjectHash)
      throw new DomainConflictError('Final brief hash does not match payload');
    return this.database.sql.begin(async (sql) => {
      const current = await runById(sql, input.runId, true);
      if (current === undefined) throw new DomainNotFoundError('run');
      if (current.version !== input.expectedVersion)
      ...
      if (input.approvalSubjectId !== undefined) {
        const quorum = (
          await sql<
            {
              ...
              rejected: number;
              mismatched: number;
              distinct_commercial_actors: number;
              distinct_commercial_authorities: number;
            }[]
          >`select
          (select count(*)::int from approval_requirement_entries where approval_subject_id = ${input.approvalSubjectId}) required,
          ...
          from approval_decisions where approval_subject_id = ${input.approvalSubjectId}`
        )[0];
        if (
          quorum === undefined ||
          quorum.required !== quorum.approved ||
          quorum.rejected !== 0 ||
          quorum.mismatched !== 0 ||
          (quorum.distinct_commercial_authorities === 2 && quorum.distinct_commercial_actors < 2)
        )
          throw new DomainConflictError('Approval quorum does not authorize this final snapshot');
      }
      await completeLease(sql, input);
      const draftVersion =
        input.approvalSubjectId === undefined
          ...
          : (
              await sql<
                { draft_version: number }[]
              >`select draft_version from approval_subjects where id = ${input.approvalSubjectId} and run_id = ${input.runId}`
            )[0]?.draft_version;
      if (draftVersion === undefined)
        throw new DomainConflictError('Approval subject is not bound to this run');
      const existing = (
        await sql<
          { subject_hash: string; payload: Record<string, unknown> }[]
        >`select subject_hash, payload from briefs where run_id = ${input.runId} and draft_version = ${draftVersion} for update`
      )[0];
      if (
        existing !== undefined &&
        (existing.subject_hash !== input.subjectHash ||
          canonicalJson(existing.payload) !== canonicalJson(input.payload))
      )
        throw new DomainConflictError('Final brief replay conflicts with persisted snapshot');
      if (existing === undefined)
        await sql`insert into briefs (id, run_id, approval_subject_id, draft_version, payload, subject_hash, finalized_at) values
      ...
      await appendEvent(sql, input.runId, 'complete', {
        version: row.version,
        subjectHash: input.subjectHash,
        deterministic: true
      });
  ...
  public async failRun(
    ...
    return this.database.sql.begin(async (sql) => {
      const current = await runById(sql, input.runId, true);
      if (current === undefined) throw new DomainNotFoundError('run');
      if (current.status === 'failed') return asRun(current);
      if (current.version !== input.expectedVersion)
        throw new DomainConflictError('Run version is stale');
      await completeLease(sql, input);
      const status = transitionRun(current.status, 'fail');
      const row = (
        await sql<
          RunRow[]
        >`update runs set status = ${status}, version = version + 1, updated_at = now() where id = ${input.runId} and version = ${input.expectedVersion} returning id, opportunity_id, requested_by, status, version, generation_provider, generation_model, start_request_hash`
