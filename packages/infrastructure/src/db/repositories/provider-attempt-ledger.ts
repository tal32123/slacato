...
import {
  ProviderAttemptFinalizationConflict,
...
type ReservationRow = Readonly<{
  ...
  status: 'reserved' | 'settled' | 'released' | 'possible_duplicate';
...
export class PostgresProviderAttemptLedger implements ProviderAttemptLedger {
  ...
  public async assertRunBudget(input: RunBudgetLimits): Promise<void> {
    ...
    const budget = (
      await this.database.sql<
        Pick<BudgetRow, 'max_calls' | 'deadline_ms' | 'deadline_at'>[]
      >`select max_calls, deadline_ms, deadline_at from run_budgets
      where run_id = ${input.scope}
        and max_calls = ${input.maxCalls}
        and deadline_ms = ${input.deadlineMs}`
    )[0];
    if (budget === undefined) {
      const exists = await this.database.sql<
        { exists: boolean }[]
      >`select exists(select 1 from run_budgets where run_id = ${input.scope}) as exists`;
      if (exists[0]?.exists === true)
        throw new Error('Run budget does not match the requested run scope');
      throw new Error('Run budget does not exist');
    }
    if (budget.deadline_ms !== input.deadlineMs)
      throw new Error('Run budget does not match the requested run scope');
    if (new Date(budget.deadline_at).getTime() <= Date.now())
      throw new Error('Shared run deadline reached');
  }
  ...
  public async remainingDeadlineMs(runScope: string): Promise<number> {
    const deadline = (
      ...
      >`select deadline_at from run_budgets where run_id = ${runScope}`
    )[0]?.deadline_at;
  ...
  public async beginAttempt(
    input: Parameters<ProviderAttemptLedger['beginAttempt']>[0]
  ): Promise<ProviderAttemptReservation> {
    const outcome = await this.database.sql.begin(async (sql) => {
      const logicalGenerationId =
        input.logicalGenerationId ??
        `generation_${createHash('sha256').update(`${input.runScope}\u0000${input.operation}`).digest('hex')}`;
      const budget = (
        await sql<
          BudgetRow[]
        >`select run_id, max_calls, deadline_ms, deadline_at, used_calls, used_input_tokens, used_output_tokens, reserved_output_tokens from run_budgets where run_id = ${input.runScope} for update`
      )[0];
      ...
      const abandoned = await sql<
        ReservationRow[]
      >`select id, attempt_id, run_id, granted_output_tokens, reserved_input_tokens, status, actual_input_tokens, actual_output_tokens, request_id, response_id, failure_category, failure_code
      ...
      const ordinalRow = (
        await sql<{ ordinal: number }[]>`select coalesce(max(ordinal), 0)::integer + 1 as ordinal
        from run_budget_reservations
        where run_id = ${input.runScope} and logical_generation_id = ${logicalGenerationId} and operation = ${input.operation}`
      )[0];
      ...
      if (refreshed === undefined || refreshed.used_calls >= refreshed.max_calls)
        throw new Error('Run call limit is exhausted');
      const grant = 1;
      ...
      return {
        reservation: { reservationId, attemptId, ordinal, grantedOutputTokens: grant },
        recovered
      };
    });
    for (const recovered of outcome.recovered) {
      logger.error({
        ...
        correlationId: recovered.row.attempt_id,
        runId: recovered.attempt.run_id,
        attemptId: recovered.row.attempt_id,
        status: 'possible_duplicate',
        provider: recovered.attempt.provider,
        model: recovered.attempt.model,
        durationMs: Math.max(0, Date.now() - new Date(recovered.attempt.started_at).getTime()),
        retryCount: recovered.attempt.ordinal - 1,
        inputTokens: recovered.row.reserved_input_tokens,
        outputTokens: recovered.row.granted_output_tokens,
    ...
    logger.info({
      ...
      correlationId: reservation.attemptId,
      runId: input.runScope,
      attemptId: reservation.attemptId,
      status: 'started',
      provider: input.provider,
      model: input.model,
      durationMs: 0,
      retryCount: reservation.ordinal - 1,
  ...
  public async settleAttempt(
    ...
    const completed = await this.database.sql.begin(async (sql) => {
      ...
      if (row.status !== 'reserved') {
        if (
          row.status === 'settled' &&
          same(row.actual_input_tokens, actualInput) &&
          same(row.actual_output_tokens, actualOutput) &&
          same(row.request_id, input.requestId) &&
          same(row.response_id, input.responseId)
        )
          return undefined;
      ...
      const attempt = (
        await sql<
          AttemptLogRow[]
        >`update generation_attempts set status = 'completed', possible_duplicate = false, request_id = ${input.requestId ?? null}, response_id = ${input.responseId ?? null}, input_tokens = ${actualInput}, output_tokens = ${actualOutput}, completed_at = now() where id = ${row.attempt_id}
        returning run_id, provider, model, ordinal, started_at`
      )[0];
      if (attempt === undefined)
        throw new ProviderAttemptFinalizationConflict('Unknown provider attempt');
    ...
    logger.info({
      ...
      runId: completed.attempt.run_id,
      ...
      retryCount: completed.attempt.ordinal - 1,
      inputTokens: completed.actualInput,
      outputTokens: completed.actualOutput
    });
  }

  /** Releases a provider-attempt reservation according to whether the request may have been sent. */
  public async releaseAttempt(
    input: Parameters<ProviderAttemptLedger['releaseAttempt']>[0]
  ): Promise<void> {
    const failed = await this.database.sql.begin(async (sql) => {
      ...
      if (row.status !== 'reserved') {
        if (
          row.status === status &&
          same(row.actual_output_tokens, output) &&
          same(row.failure_category, input.category) &&
          same(row.failure_code, input.diagnosticCode)
        )
          return undefined;
        throw new ProviderAttemptFinalizationConflict();
      }
      ...
      const attempt = (
        await sql<
          AttemptLogRow[]
        >`update generation_attempts set status = ${input.disposition === 'safe_not_sent' ? 'failed' : 'possible_duplicate'}, possible_duplicate = ${input.disposition === 'possibly_sent'}, output_tokens = ${output}, completed_at = now() where id = ${row.attempt_id}
        returning run_id, provider, model, ordinal, started_at`
      )[0];
      if (attempt === undefined)
        throw new ProviderAttemptFinalizationConflict('Unknown provider attempt');
    ...
    const diagnosticCode = input.diagnosticCode
      ?.toUpperCase()
      .replace(/[^A-Z0-9_]/g, '_')
      .slice(0, 128);
    logger.error({
      event: 'provider_attempt_failed',
      correlationId: input.attemptId,
      runId: failed.attempt.run_id,
      attemptId: input.attemptId,
      status: input.disposition === 'safe_not_sent' ? 'failed' : 'possible_duplicate',
      ...
      retryCount: failed.attempt.ordinal - 1,
      inputTokens: failed.inputTokens,
      outputTokens: failed.output ?? 0,
      errorCode:
        diagnosticCode === undefined || diagnosticCode.length === 0
  ...
  public async recordAttemptMetadata(
    ...
    >`select output_mode, validation_attempts, validation_issues, warnings from generation_attempts where id = ${input.attemptId}`;
    const existing = rows[0];
    if (existing === undefined)
      throw new ProviderAttemptFinalizationConflict('Unknown provider attempt');
    if (existing.output_mode !== null) {
      if (
        existing.output_mode === input.outputMode &&
        existing.validation_attempts === input.validationAttempts &&
        JSON.stringify(existing.validation_issues) === JSON.stringify(input.validationIssues) &&
        JSON.stringify(existing.warnings) === JSON.stringify(input.warnings)
      )
        return;
      throw new ProviderAttemptFinalizationConflict(
        'Provider attempt metadata was already recorded differently'
      );
    }
    await this.database
      .sql`update generation_attempts set output_mode = ${input.outputMode}, validation_attempts = ${input.validationAttempts},
  ...
  private async lockReservation(
    sql: QuerySql,
    input: ProviderAttemptReservation
  ): Promise<ReservationRow> {
    const row = (
      await sql<
        ReservationRow[]
      >`select id, attempt_id, run_id, granted_output_tokens, reserved_input_tokens, status, actual_input_tokens, actual_output_tokens, request_id, response_id, failure_category, failure_code from run_budget_reservations where id = ${input.reservationId} and attempt_id = ${input.attemptId} for update`
    )[0];
    if (row === undefined)
      throw new ProviderAttemptFinalizationConflict('Unknown provider attempt reservation');
    return row;
  }

  /** Finalizes an abandoned reservation as a possible duplicate while charging its reserved output. */
  private async finalizePossibleDuplicate(
    sql: QuerySql,
    row: ReservationRow
    ...
    const attempt = (
      await sql<
        AttemptLogRow[]
      >`update generation_attempts set status = 'possible_duplicate', possible_duplicate = true, output_tokens = ${row.granted_output_tokens}, completed_at = now() where id = ${row.attempt_id}
      returning run_id, provider, model, ordinal, started_at`
    )[0];
    if (attempt === undefined)
      throw new ProviderAttemptFinalizationConflict('Unknown provider attempt');
