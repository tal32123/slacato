import { z } from 'zod';
// biome-ignore lint/correctness/noUnusedImports: Required for declaration emission of branded IDs.
...
import {
  ...
  withSerializedByteLimit
} from '../shared/serialized-size.js';
...
const evidenceManifestIdSchema = z
  .string()
  .min(1)
  .max(128)
  .regex(/^manifest_[A-Za-z0-9][A-Za-z0-9_-]*$/);

/** Stable, bounded reference from a claim to an immutable authorized evidence version. */
export const citationSchema = immutableSchema(
  z
    .object({
      id: citationIdSchema,
...
export const claimSchema = immutableSchema(
  z
    .object({
      id: claimIdSchema,
...
export const reviewWarningSchema = immutableSchema(
  z
    .object({
      code: z
        .string()
...
export const stakeholderSchema = immutableSchema(
  z
    .object({
...
export const evidenceSummarySchema = immutableSchema(
  z
    .object({
      evidenceId: evidenceIdSchema,
...
const buyerGoalsAndBusinessDriversSchema = z
  .object({
    goals: z.array(shortTextSchema).max(MAX_LIST_ITEMS),
...
const negotiationStateSchema = z
  .object({
...
const recommendedNextActionsSchema = z
  .object({
...
const confidenceAndReviewWarningsSchema = z
  .object({
 ...
 */
export const dealBriefSchema = immutableSchema(
  withSerializedByteLimit(
    z
      .object({
...
export const conversationArtifactSchema = immutableSchema(
  withSerializedByteLimit(
    z
      .object({
...
export const stakeholderArtifactSchema = immutableSchema(
  withSerializedByteLimit(
    z
      .object({
        evidenceManifestId: evidenceManifestIdSchema,
...
export const commercialArtifactSchema = immutableSchema(
  withSerializedByteLimit(
    z
      .object({
        evidenceManifestId: evidenceManifestIdSchema,
