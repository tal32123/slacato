...
function normalizedIssues(error: z.ZodError, candidate: unknown): readonly NormalizedValidationIssue[] {
  return error.issues.map((issue) => ({
    path: issue.path.map(String).join('.'),
    code: issue.code,
    message: (() => {
      const field = issue.path.at(-1);
      const received = typeof field === 'string' && ['id', 'evidenceId', 'locator'].includes(field)
...
export function createBudgetedModelGateway(transport: ModelTransport, contextPolicy: ContextWindowPolicy | undefined = undefined, attemptLedger: ProviderAttemptLedger): BudgetedModelGateway {
  return {
    async generateObject<Value>(request: GenerateObjectRequest<Value>): Promise<GenerationResult<Value>> {
      ...
      const prepared = request.context === undefined || policy === undefined ? undefined : policy.prepare(request.context);
      ...
      while (true) {
        ...
        const messages = policy === undefined ? candidateMessages : prepared === undefined
