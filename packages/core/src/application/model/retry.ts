...
/** Reads an unknown failure as a record when possible. */
...
export class RunBudgetLedger implements SharedRunBudget {
  ...
  /** Adds any input tokens consumed beyond the amount reserved before the call. */
  ...
  /** Finalizes a reservation with the provider’s actual output usage. */
...
export class BoundedRetryController {
  ...
  public settleAttempt(reservation: AttemptReservation, actualOutputTokens: number | undefined): void {
    ...
    if (sharedFailure !== undefined) throw new OutputBudgetOverrunError('Shared provider-attempt accounting failed', sharedFailure);
  }
