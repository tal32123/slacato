...
function takeTokens(content: string, tokenLimit: number): string {
  return content.slice(0, Math.max(0, tokenLimit * CHARS_PER_TOKEN));
}
/** Copies a model message without sharing its object identity. */
function copy(message: ModelMessage): ModelMessage {
...
function isRepairFeedback(message: ModelMessage): boolean {
  return (
    message.content.includes('BEGIN_UNTRUSTED_INVALID_OUTPUT') &&
...
function requireInvariant(label: string, content: string, budget: number): void {
  if (estimateTokens(content) > budget)
    throw new ContextBudgetError(`${label} invariant exceeds its section budget`);
}
...
function renderRequiredEvidence(sections: readonly ContextSection[], budget: number): string {
  ...
  if (requiredTokens > budget)
    throw new ContextBudgetError('Evidence citation ID invariant exceeds its section budget');
  ...
  for (const [index, section] of sections.entries()) {
    ...
    if (sealedEnvelope && estimateTokens(section.content) > remaining)
      throw new ContextBudgetError('Sealed evidence envelope exceeds its section budget');
...
export class ContextWindowPolicy {
  ...
  public constructor(private readonly settings: ContextWindowSettings) {
    if (settings.contextWindowTokens <= settings.reservedOutputTokens)
      throw new ContextBudgetError('Context window must exceed reserved output capacity');
  }
  ...
  public prepare(input: ContextWindowInput): PreparedContext {
    ...
    const configuredTokens =
      ...
      budgets.artifacts +
      budgets.history;
    if (configuredTokens > availableTokens)
      throw new ContextBudgetError('Section budgets exceed available input capacity');
    ...
    if (invariantTokens > availableTokens)
      throw new ContextBudgetError('Required context invariants exceed available input capacity');
    ...
    for (const message of [...(input.history ?? [])].reverse()) {
      ...
      const content = message.content.slice(
        Math.max(0, message.content.length - remaining * CHARS_PER_TOKEN)
      );
    ...
    if (inputTokens > availableTokens)
      throw new ContextBudgetError('Prepared context exceeds available input capacity');
    return {
      messages,
      invariantMessages,
      optionalMessages,
  ...
  public rebudget(
    ...
    supplemental: readonly ModelMessage[]
    ...
    if (supplementalTokens > availableTokens - invariantTokens)
      throw new ContextBudgetError('Complete repair feedback exceeds available input capacity');
    ...
    if (messageTokens(result) > availableTokens)
      throw new ContextBudgetError('Rebudgeted context exceeds available input capacity');
    return result;
  }
  ...
  public rebudgetRaw(messages: readonly ModelMessage[]): readonly ModelMessage[] {
    ...
    if (messages.some(isRepairFeedback)) {
      if (messageTokens(messages) > availableTokens)
        throw new ContextBudgetError('Complete repair feedback exceeds available input capacity');
    ...
    if (
      messageTokens(retained) >
      this.settings.contextWindowTokens - this.settings.reservedOutputTokens
    )
      throw new ContextBudgetError('Rebudgeted context exceeds available input capacity');
    return retained;
  }
...
function validBindings(value: unknown): value is ContextCheckpointBindings {
  ...
  return [
    ...
    (key) => Object.hasOwn(record, key) && typeof record[key] === 'string' && record[key].length > 0
  );
}
...
function assertCheckpoint(
  value: unknown,
  input: BoundedCompactionInput
): asserts value is ContextCheckpoint {
  const record = asPlainRecord(value);
  if (
    record === undefined ||
    !Object.hasOwn(record, 'coveredMessageRange') ||
    !Object.hasOwn(record, 'summary') ||
    !Object.hasOwn(record, 'validationState')
  )
    throw new ContextBudgetError('Compactor returned a malformed checkpoint');
  const checkpoint = value as ContextCheckpoint;
  if (
    !validBindings(checkpoint) ||
    checkpoint.summary.length === 0 ||
    checkpoint.validationState !== 'validated'
  )
    throw new ContextBudgetError('Compactor returned an unvalidated checkpoint');
  const range = asPlainRecord(checkpoint.coveredMessageRange);
  if (
    range === undefined ||
    !Object.hasOwn(range, 'from') ||
    !Object.hasOwn(range, 'to') ||
    !Number.isInteger(checkpoint.coveredMessageRange.from) ||
    !Number.isInteger(checkpoint.coveredMessageRange.to) ||
    checkpoint.coveredMessageRange.from !== input.coveredMessageRange.from ||
    checkpoint.coveredMessageRange.to !== input.coveredMessageRange.to
  )
    throw new ContextBudgetError('Compactor returned an invalid covered message range');
  if (!isContextCheckpointReusable(checkpoint, input.bindings))
    throw new ContextBudgetError('Compactor checkpoint bindings do not match the request');
}

/** Creates a one-step history compactor that cannot recursively invoke itself. */
export function createNonRecursiveContextCompactor(
  gateway: NonRecursiveCompactionGateway,
  policy: ContextWindowPolicy
): ContextCompactor {
  return {
    async compact(input: BoundedCompactionInput): Promise<ContextCheckpoint> {
      if (input.priorInvocations >= 1)
        throw new ContextBudgetError('Repeated compaction is not allowed');
      if (input.maxSteps !== 1 || input.maxRetries !== 0)
        throw new ContextBudgetError('Compaction permits exactly one step and no retries');
      if (input.maxInputTokens <= 1 || input.maxOutputTokens <= 0)
        throw new ContextBudgetError('Compaction input/output limits are invalid');
      if (
        input.maxInputTokens > MAX_COMPACTION_INPUT_TOKENS ||
        input.maxOutputTokens > MAX_COMPACTION_OUTPUT_TOKENS
      )
        throw new ContextBudgetError('Compaction token limits exceed the hard safety cap');
      const prepared = policy.prepare({ ...input.context, history: input.history });
      if (prepared.inputTokens > input.maxInputTokens)
        throw new ContextBudgetError('Prepared compaction input exceeds maxInputTokens');
      const checkpoint = await gateway.compact({
        mode: 'non_recursive',
        context: prepared,
      ...
      if (estimateTokens(JSON.stringify(checkpoint)) > input.maxOutputTokens)
        throw new ContextBudgetError('Compactor output exceeds maxOutputTokens');
      return checkpoint;
    }
  };
}
...
export function isContextCheckpointReusable(
  ...
  current: ContextCheckpointBindings
): boolean {
  return (
    checkpoint.validationState === 'validated' &&
    validBindings(checkpoint) &&
    checkpoint.scopeHash === current.scopeHash &&
