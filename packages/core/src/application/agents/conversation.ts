...
const TASK =
  'Extract buyer goals, concerns, commitments, objections, and missing context. Every factual claim must copy one complete supplied citation tuple without changing any of its three fields. Return only the strict conversation artifact.';
  ...
  public async run(context: AgentContext): Promise<ConversationArtifact> {
    const result = await runAgent({
      ...
      validate: (value, evidence) =>
        validateConversationArtifact(value, context.manifest.id, evidence)
    });
