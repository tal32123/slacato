...
export interface DealBriefWorkflowServices {
  ...
  conversation(run: WorkflowRun, context: DealBriefRetrievalContext, invocationId: string): Promise<ConversationArtifact>;
  stakeholder(run: WorkflowRun, context: DealBriefRetrievalContext, invocationId: string): Promise<StakeholderArtifact>;
  commercial(run: WorkflowRun, context: DealBriefRetrievalContext, invocationId: string): Promise<CommercialArtifact>;
  strategy(
    run: WorkflowRun,
    input: Readonly<{ context: DealBriefRetrievalContext } & StrategyArtifacts>,
    invocationId: string
  ): Promise<DealBrief>;
  approvalInput(run: WorkflowRun, brief: DealBrief, commercial: CommercialArtifact): ApprovalRequirementInput | Promise<ApprovalRequirementInput>;
  validateDraft(payload: unknown): DealBrief | Promise<DealBrief>;
}
export type StartDealBriefCommand = Readonly<{ opportunityId: string; requestedBy: string; idempotencyKey: string }>;
...
function retrievalContextOf(
  ...
  return valueOf(checkpoint, 'retrieval') as DealBriefRetrievalContext;
}
...
function specialistArtifactOf(
  ...
  const value = valueOf(checkpoint, name);
  const shouldParse = checkpoint?.status === 'degraded' || Object.hasOwn(value, 'evidenceManifestId');
...
export function assertApprovableBrief(value: unknown): DealBrief {
  ...
  if (!hasRetainedApprovalContent) throw new DomainValidationError('Approval payload has no retained deal content');
  return parsed;
}
...
export class StartDealBrief {
  ...
  public async execute(input: StartDealBriefCommand): Promise<RunId> {
    ...
    const requestHash = hashApprovalPayload({ opportunityId: input.opportunityId, requestedBy: input.requestedBy, idempotencyKey: input.idempotencyKey, generationProvider: this.model.provider, generationModel: this.model.model });
...
export class ProcessDealBriefStep {
  ...
  private async specialists(run: WorkflowRun, lease: StepLease, causal: WorkflowCommand) {
    ...
    const checkpoints = await Promise.all(names.map(async (name) => ({
      name,
  ...
  private async synthesize(run: WorkflowRun, lease: StepLease, causal: WorkflowCommand) {
    ...
    const draftVersion = typeof causal.payload.draftVersion === 'number'
      ? causal.payload.draftVersion
      : 1;
    ...
    try {
      const value = existing?.value ?? await this.services.strategy(
        run,
  ...
  private async validate(run: WorkflowRun, lease: StepLease, causal: WorkflowCommand) {
    const draftVersion = typeof causal.payload.draftVersion === 'number' ? causal.payload.draftVersion : 1;
    const strategy = await this.store.getCheckpoint({ runId: run.id, step: `strategy:${draftVersion}` });
    const commercial = specialistArtifactOf(
      await this.store.getCheckpoint({ runId: run.id, step: 'specialist:commercial' }),
      'commercial'
    );
    ...
    try {
      ...
      requirement = decideApprovalRequirement(await this.services.approvalInput(run, payload, commercial));
    } catch (error) {
