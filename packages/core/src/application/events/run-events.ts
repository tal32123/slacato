...
function spansOfKind<K extends TraceSpan['kind']>(
  spans: readonly TraceSpan[],
  kind: K
): TraceFor<K>[] {
...
function assertAttemptEvidence(spans: readonly TraceSpan[], attempt: TraceSpan): void {
  const models = spansOfKind(spans, 'model_call').filter(
    (span) => span.step === attempt.step && span.parentSpanId === attempt.spanId
  );
  if (models.length === 0)
    throw new TraceCompletenessError(`Trace is missing model call for ${attempt.step}`);
  const ordinals = new Set<number>();
  for (const model of models) {
    if (ordinals.has(model.data.ordinal))
      throw new TraceCompletenessError(`Trace has duplicate model ordinal for ${attempt.step}`);
    ordinals.add(model.data.ordinal);
    const validation = spansOfKind(spans, 'validation').find(
      (span) => span.step === attempt.step && span.parentSpanId === model.spanId
    );
    if (validation === undefined)
      throw new TraceCompletenessError(
        `Trace is missing validation for ${attempt.step} attempt ${model.data.ordinal}`
      );
    if (
      !spansOfKind(spans, 'guardrail').some(
        (span) => span.step === attempt.step && span.parentSpanId === model.spanId
      )
    ) {
      throw new TraceCompletenessError(
        `Trace is missing guardrail for ${attempt.step} attempt ${model.data.ordinal}`
      );
    }
    if (
      !spansOfKind(spans, 'usage').some(
        (span) => span.step === attempt.step && span.parentSpanId === model.spanId
      )
    ) {
      throw new TraceCompletenessError(
        `Trace is missing usage for ${attempt.step} attempt ${model.data.ordinal}`
      );
    }
    if (
      validation.data.validationAttempts > 0 &&
      !spansOfKind(spans, 'repair').some(
        (span) => span.step === attempt.step && span.parentSpanId === model.spanId
      )
    ) {
      throw new TraceCompletenessError(
        `Trace is missing repair for ${attempt.step} attempt ${model.data.ordinal}`
      );
    }
  }
}
...
export function assertTraceComplete(runId: string, input: readonly TraceSpan[]): void {
  ...
  if (spans.some((span) => span.runId !== runId))
    throw new TraceCompletenessError('Trace contains cross-run spans');
  const byId = new Map<string, TraceSpan>();
  for (const span of spans) {
    if (byId.has(span.spanId))
      throw new TraceCompletenessError(`Trace has duplicate span ${span.spanId}`);
  ...
  if (authorizations.length === 0)
    throw new TraceCompletenessError('Trace is missing authorization lookup');
  const denied = authorizations.some(
    (span) => span.status === 'denied' || span.data.decision === 'denied'
  );
  if (denied) {
    if (spans.some(({ kind }) => kind !== 'authorization_lookup')) {
      throw new TraceCompletenessError(
        'Denied trace contains evidence, agent, citation, recommendation, or locator facts'
      );
    }
    if (
      authorizations.some((span) => span.status !== 'denied' || span.data.decision !== 'denied')
    ) {
  ...
  if (
    !authorizations.some((span) => span.status === 'completed' && span.data.decision === 'allowed')
  ) {
  ...
  for (const partial of partialFailures) {
    ...
    if (
      ...
      triggering.status !== 'degraded' ||
      !['specialist_attempt', 'strategy_attempt'].includes(triggering.kind)
    ) {
      throw new TraceCompletenessError(
        'Partial decision is not linked to its degraded triggering attempt'
      );
    }
  ...
  for (const attempt of attempts.filter((span) => span.status === 'degraded')) {
    if (!partialFailures.some((span) => span.parentSpanId === attempt.spanId)) {
      throw new TraceCompletenessError(
        `Degraded attempt ${attempt.step} is missing its linked partial decision`
      );
    }
  }
  ...
  for (const specialist of ['conversation', 'stakeholder', 'commercial']) {
    const attempt = spans.find(
      (span) => span.kind === 'specialist_attempt' && span.step === specialist
    );
    if (attempt === undefined)
      throw new TraceCompletenessError(`Trace is missing ${specialist} specialist attempt`);
  ...
  for (const requirement of requirements) {
    if (
      typeof requirement.data.entryId !== 'string' ||
      ...
      throw new TraceCompletenessError(
        'Approval requirement is missing subject, entry, category, or eligible authorities'
      );
    }
  }
  ...
  if (finalization === undefined) {
    if (requirements.length === 0)
      throw new TraceCompletenessError('Non-terminal trace has no approval requirement');
    return;
  }
  const decisions = spansOfKind(spans, 'approval_decision');
  if (requirements.length > 0 && decisions.length === 0)
    throw new TraceCompletenessError('Trace is missing approval decision');
  for (const decision of decisions) {
    ...
    if (
      typeof decision.data.entryId !== 'string' ||
      typeof decision.data.subjectHash !== 'string' ||
      typeof decision.data.category !== 'string' ||
      typeof decision.data.authority !== 'string' ||
      typeof decision.data.decision !== 'string'
    ) {
      throw new TraceCompletenessError(
        'Approval decision is missing subject, entry, category, authority, or decision'
      );
    }
  }
}
