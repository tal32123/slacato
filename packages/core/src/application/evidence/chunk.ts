...
function transcriptWindows(content: string): string[] {
  ...
  const hasTranscriptMetadata =
    ...
    ['Opportunity', 'Account', 'Date', 'Source access level'].every((label) =>
      new RegExp(`^\\*\\*${label}:\\*\\*\\s*\\S+`, 'mi').test(metadataCandidate)
    );
...
export function buildEvidenceDocuments(fixtures: FixtureSet): EvidenceDocument[] {
  ...
  const opportunityById = new Map(
    fixtures.opportunities.map((opportunity) => [opportunity.opportunityId, opportunity])
  );
  ...
  for (const opportunity of fixtures.opportunities) {
    ...
    opportunitiesByAccount.set(
      opportunity.accountId,
      [...existing, opportunity].sort((left, right) =>
  ...
  const add = (
    ...
    documents.push({
      ...
      policyHash: classification.policyHash
    });
  };

  for (const account of fixtures.accounts)
    for (const opportunity of opportunitiesByAccount.get(account.accountId) ?? [])
      add(
        {
          externalId: `${account.accountId}:${opportunity.opportunityId}:account`,
          sourceType: 'salesforce',
          accountId: account.accountId,
          opportunityId: opportunity.opportunityId,
          reliability: 'authoritative_system',
          sourceLocator: `salesforce/accounts.tsv#${account.accountId}/opportunity/${opportunity.opportunityId}`,
          content: recordContent(account)
        },
    ...
    add(
      {
        ...
        opportunityId: opportunity.opportunityId,
        reliability: 'authoritative_system',
        sourceLocator: `salesforce/opportunities.tsv#${opportunity.opportunityId}`,
        content: recordContent(opportunity)
      },
      { sourceType: 'salesforce' },
      opportunity
    );
  for (const contact of fixtures.contacts) {
    ...
    for (const opportunity of opportunitiesByAccount.get(contact.accountId) ?? [])
      add(
        {
          externalId: `${contact.contactId}:${opportunity.opportunityId}:contact`,
          sourceType: 'salesforce',
          accountId: contact.accountId,
          opportunityId: opportunity.opportunityId,
          eventDate: contact.lastInteractionDate,
          reliability: 'authoritative_system',
          sourceLocator: `salesforce/contacts.tsv#${contact.contactId}/opportunity/${opportunity.opportunityId}`,
          content: recordContent(contact)
        },
        { sourceType: 'salesforce', sourceAccessLevel: account?.accessLevel },
        opportunity
      );
  }
  for (const summary of fixtures.gongSummaries)
    add(
      {
        externalId: `${summary.callId}:summary`,
        sourceType: 'gong_summary',
        accountId: summary.accountId,
        opportunityId: summary.opportunityId,
        eventDate: summary.callDate,
        reliability: 'conversation_summary',
        sourceLocator: `gong/gong_call_summaries.tsv#${summary.callId}`,
        content: recordContent(summary)
      },
      { sourceType: 'gong_summary', sourceAccessLevel: summary.sourceAccessLevel },
      opportunityById.get(summary.opportunityId)
    );
  for (const transcript of fixtures.transcripts)
    add(
      {
        externalId: `${transcript.callId}:transcript`,
        sourceType: 'gong_transcript',
        accountId: transcript.accountId,
        opportunityId: transcript.opportunityId,
        eventDate: transcript.callDate,
        reliability: 'direct_conversation',
        sourceLocator: transcript.sourceLocator,
        content: transcript.content
      },
      { sourceType: 'gong_transcript', sourceAccessLevel: transcript.sourceAccessLevel },
      opportunityById.get(transcript.opportunityId)
    );
  for (const note of fixtures.pricingNotes) {
    const opportunity = opportunityById.get(note.opportunityId);
    if (opportunity === undefined)
      throw new Error(`Unknown pricing opportunity ${note.opportunityId}`);
    add(
      {
        externalId: note.pricingNoteId,
        sourceType: 'pricing',
        accountId: opportunity.accountId,
        opportunityId: note.opportunityId,
        reliability: 'authoritative_system',
        sourceLocator: `pricing/pricing_notes.tsv#${note.pricingNoteId}`,
        content: recordContent(note)
      },
      {
        sourceType: 'pricing',
  ...
  for (const update of fixtures.slackUpdates)
    add(
      {
        externalId: update.updateId,
        sourceType: 'slack',
        accountId: update.accountId,
        opportunityId: update.opportunityId,
        eventDate: update.updateDate,
        reliability: 'internal_collaboration',
        sourceLocator: `slack/account_team_updates.tsv#${update.updateId}`,
        content: recordContent(update)
      },
      { sourceType: 'slack', sourceAccessLevel: update.sourceAccessLevel },
      opportunityById.get(update.opportunityId)
    );
  for (const opportunity of fixtures.opportunities)
    add(
      {
        externalId: `deal-desk-policy:${opportunity.opportunityId}`,
        sourceType: 'policy',
        accountId: opportunity.accountId,
        opportunityId: opportunity.opportunityId,
        reliability: 'authoritative_policy',
        sourceLocator: `policies/deal_desk_policy.md#opportunity/${opportunity.opportunityId}`,
