...
export const approvalBriefPayloadSchema = z
  .object({
    dealSnapshot: z
      .object({
      ...
      .strict(),
    executiveSummary: z
      .object({ narrative: sectionTextSchema, claims: z.array(claimSchema).max(50).optional() })
      .strict(),
    buyerGoalsAndBusinessDrivers: z
      .object({
        goals: z.array(shortTextSchema).max(50),
        businessDrivers: z.array(shortTextSchema).max(50),
        claims: z.array(claimSchema).max(50).optional()
      })
      .strict(),
    stakeholderMap: z
      .object({
        ...
        coverageGaps: z.array(shortTextSchema).max(50).optional(),
        claims: z.array(claimSchema).max(50).optional()
      })
      .strict(),
    negotiationState: z
      .object({
        ...
        leverage: z.array(shortTextSchema).max(50).optional(),
        risks: z.array(shortTextSchema).max(50),
        claims: z.array(claimSchema).max(50).optional()
      })
      .strict(),
...
export const approvalDecisionResultSchema = z
  ...
  .strict();
export const approvalFieldDiffSchema = z
  .object({
  ...
  .strict();
export const approvalStructuredDiffSchema = z
  .object({
...
export const approvalDecisionViewSchema = z
  .object({
...
export const approvalInboxEntrySchema = z
  .object({
    approvalSubjectId: opaqueIdSchema,
    runId: opaqueIdSchema,
    runVersion: z.number().int().nonnegative(),
    ...
    quorum: z
      .object({ completed: z.number().int().nonnegative(), required: z.number().int().positive() })
      .strict(),
...
export const approvalInboxResponseSchema = z
  .object({
...
export const approvalRequirementViewSchema = z
  .object({
    entryId: opaqueIdSchema,
    category: approvalCategorySchema,
    policyTriggers: z.array(policyTriggerSchema).max(20),
...
export const approvalDetailResponseSchema = z
  .object({
    sessionVersion: z.string().min(1).max(256),
    approvalSubjectId: opaqueIdSchema,
    runId: opaqueIdSchema,
