...
Slack fixtures are generated once per release candidate with live Ollama Cloud assistance into a same-filesystem temporary staging directory, validated by deterministic TypeScript rules, and reviewed before canonical files change. The staged metadata records real provider/model identifiers, capability/output mode, nonzero call/token usage, prompt/schema/input/output hashes, generation time, reviewer status, and row categories. Only an approved, revalidated TSV+metadata bundle may be promoted under an exclusive fixture lock using atomic directory-level rename with rollback; any pre-promotion failure leaves the previous canonical fixture byte-for-byte intact, and ingestion rejects partial or cross-hash-mismatched bundles. Submission preparation then reingests and reindexes the promoted live TSV and binds sample evidence manifests to its content/version hash. Runtime ingestion consumes that reviewed fixture; it does not regenerate Slack messages for each run. The live verifier rejects `mock`, replay, fake, zero-usage, missing-provenance, stale-ingestion, or sample/fixture hash mismatch markers.
