...
- Part 1 uses PostgreSQL FTS, adaptive exact/HNSW semantic retrieval, and RRF. Exact cosine remains the oracle and fallback; HNSW activates only for large promoted security domains after authorization, recall, underfill, latency, and operational gates pass. pg_trgm and cross-encoder reranking remain deferred until baseline metrics justify them.
...
The architecture, AI/RAG, and UI/UX reviews resolved these choices: NestJS plus BullMQ and a PostgreSQL transactional outbox instead of in-request work or a general workflow platform; adaptive authorized exact/HNSW retrieval behind one deep module, with no global shared graph and exact reranking/fallback; capability-aware native schema or prompted-JSON generation; immutable versioned approval subjects; provider-neutral context budgeting; read-only diagnostics instead of model/theme controls; and mandatory live artifacts before packaging.
...
Create the `vector` extension. Add normalized tables for personas, permission grants, accounts, opportunities, contacts, immutable document/evidence versions, immutable run evidence manifests, runs, outbox commands, leased step invocations, checkpoints, generation attempts, context checkpoints, specialist artifacts, claims, citations, approval subjects/decisions, briefs, trace spans, run events, and audit events. Use a dimension-flexible authoritative vector column and persist provider/model/dimension/profile metadata; reject mixed-profile comparisons in application invariants. Do not pin a pgvector typmod to the mock profile or an unverified Ollama dimension. Add embedding-profile, security-domain, and search-index-generation metadata so Task 6 can build a rebuildable fixed-dimension search projection after the live profile probe. Use a generated `tsvector`. Fixture-sized and selective domains remain exact; HNSW activation requires an eligible promoted domain and explicit pre-production gates.
...
### Task 6: Build Authorized Hybrid Retrieval with Adaptive Exact/HNSW, FTS, and RRF
...
**Interfaces:**
- Produces: deep module `EvidenceAccess.retrieve({ runId, query, limit }): Promise<RetrievalResult>` and `EvidenceAccess.resolveCitation(...)`; callers cannot supply authorization scope or ANN settings. Exact and HNSW candidate adapters, routing, reauthorization, reranking, and fallback remain private; `reciprocalRankFusion(lists, k): RankedId[]` remains deterministic.
...
- [ ] **Step 3: Implement embedding ingestion, profile projection, and health validation**

Use `embedMany` with the adapter's AI SDK 7 embedding factory verified in Task 7. Batch documents into the dimension-flexible authoritative store, persist model/dimension/normalization/content hashes, refuse mixed versions, and skip unchanged chunks. After the live profile probe, build a rebuildable fixed-dimension full-precision search projection and generation metadata. Classify stable ACL fingerprints into security domains; keep restricted-account and sensitive-pricing lanes separate. Build HNSW only for large eligible promoted domains, blue/green by profile and generation.

- [ ] **Step 4: Implement permission-scoped parallel retrieval**

Build an `EvidencePlan`: exact authorized opportunity/account/contact lookups, fixed section/source queries, per-source top-k, total context budget, mandatory policy inclusion, deduplication, and missing-source diagnostics. Recompute scope from persisted run/requester state before embedding or search. Issue lexical search concurrently with semantic search. Route small/selective domains to exact cosine and eligible promoted domains to HNSW; never use one global graph. HNSW uses pgvector iterative scans and bounded overfetch to produce candidate IDs only. Rejoin every candidate through the complete current authorization relation, then rerank with exact full-precision cosine. Every SQL path retains account, opportunity, source-type, effective-access, sensitive-pricing, and embedding-profile predicates. Fuse stable IDs with RRF using `score = Σ 1/(60 + rank)` and apply documented bounded reliability/recency adjustments. Mandatory policy and exact CRM grounding never depend on ANN.

Immediately reauthorize and persist an immutable `RunEvidenceManifest` containing authorized chunk version IDs/hashes, locators, classification, ranks/scores, query hash, embedding profile, selected adapter, index generation, HNSW settings hash, candidate count, exact-rerank version, fallback mode, policy hash, and originating scope hash. All specialists, repairs, validators, approvals, and exports use this manifest and never silently substitute re-ingested chunks. A deliberate refresh creates a new run/manifest version.
...
- [ ] **Step 6: Verify golden retrieval, ANN safety/recall, fallback, and query plans**

Run: `pnpm test:integration -- retrieval.test.ts && pnpm tsx scripts/evaluate.ts retrieval`  
Expected: permission and metadata leakage is 0; mandatory policy inclusion and citation/manifest authorization are 100%; exact results remain the oracle for every authorized scope; HNSW shadow recall@20 is at least 0.95 overall and 0.98 for commercial/policy-sensitive cohorts; final hybrid overlap@20 is at least 0.97; candidate underfill is below 0.1%; HNSW provides at least 2x p95 semantic-query latency improvement after exact reranking; selective/small domains, underfill, index failure, and the kill switch use exact fallback; query plans never select a global or wrong-profile graph.
...
Compute precision@k, recall@k, citation resolution/authorization, claim support, required-section completeness, policy trigger/quorum correctness, injection/unsafe-language containment, source utilization, trace completeness, Slack novelty/authorization/impact, permission leakage, HNSW underfill, and adaptive-strategy latency. Build `source-coverage.json` with exactly eight entries containing `sourceGroup`, inventory count/hash, authorized retrieval/control trace ID, expected and observed agent IDs, resulting section/claim/citation label, `expectedUtilizationRule`, and status; sanitize source bodies, prompts, denied locators, and restricted metadata. Hard gates are leakage `= 0`, citation resolution and required visible path/stable-ID labels `= 100%`, critical/material claim support `= 100%`, required sections `= 100%`, policy/approval correctness `= 100%`, required trace stages `= 100%`, all eight source-utilization entries and every required stage `= pass`, Slack PII violations `= 0`, context overflow `= 0`, initial-release compactor calls `= 0`, and retry/call-budget violations `= 0`. For every HNSW-eligible domain, additionally require semantic recall@20 versus exact `>= 0.95` overall and `>= 0.98` for commercial/policy-sensitive cohorts, final hybrid overlap@20 `>= 0.97`, candidate underfill `< 0.1%`, and p95 semantic-query latency at least `2x` faster after exact reranking. Test selective-filter underfill, final reauthorization, wrong-profile/index rejection, exact fallback, and the kill switch. Inventory alone is never sufficient. Freeze thresholds before activation.
...
Document module boundaries, deterministic workflow, transactional outbox/reconciler, invocation leases, at-least-once inference, BullMQ retry/dead-letter behavior, context budgeting without unnecessary initial-release compaction, provider capability modes, authorization-before-retrieval, authorization-lookups-versus-evidence trace separation, RRF, adaptive exact/HNSW routing, promoted security domains, filtering/iterative scans, exact reranking/fallback, index generations and activation gates, cross-process SSE replay, immutable category/quorum approval, exact environment variables, Ollama Cloud setup/model IDs/inference parameters, migrations, ingestion/indexing, tests, Promptfoo, logs, Docker, Railway, and troubleshooting. Create an explicit logical Mermaid diagram showing agents, orchestration, RAG, state, human approvals, guardrails, observability, and output channels, and a separate deployment Mermaid diagram showing web/API/worker, PostgreSQL/pgvector, Redis, secrets, model gateway, monitoring, and trust boundaries. The technical overview explains the production delta for scalability, high availability, monitoring, secrets, backups/retention, HNSW build/WAL/vacuum/replica-lag operations, exact-mode kill switching, incident/dead-letter operations, and support ownership. `docs/security.md` explains default-deny authorization, source sensitivity, approval authority/quorum, prompt injection, unsafe language, citation authorization, log redaction, secrets, ANN timing/recall noninterference limitations, and residual risks. Railway uses a public web service and private API/worker/PostgreSQL/pgvector/Redis services, bounded pools and concurrency, health checks, backups, explicit migrations, outbox-age/queue-depth/dead-letter alerts, and retention policies. Record exact build/start commands, private targets, proxy rules, preview/custom-domain behavior, and deployed smoke tests. Vercel Workflow and Eve are documented as considered but unnecessary for this bounded workflow.
