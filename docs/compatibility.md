...
Before submission, run the credentialed provider probe and regenerate `fixtures/cato/slack/account_team_updates.tsv` with a supported live provider (`openrouter` or `ollama`). Its `generation.json` must record the provider, exact model/output mode, provider request IDs when available, nonzero call and token usage, prompt/schema/source/output hashes, generation time, reviewer approval, and a row-level `reinforces | adds_context | ambiguity_conflict` category for every update. Reingest and reindex the approved fixture before generating sample runs; every sample evidence manifest must bind to that fixture content hash.

The repository does not yet contain an automated live-artifact verifier. Before submission, add one that fails on `mock`, replay, fake or zero-usage provenance, missing row categories, stale ingestion, and Slack fixture/sample hash mismatches. The checked-in Slack fixture records live OpenRouter provenance; sample-run hash binding still requires verification.
