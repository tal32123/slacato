...
test.describe('guided tour: step navigation edge cases', () => {
  ...
  test('Back from the first non-required step returns to the required login step and its route', async ({ page }) => {
    await startTourAsMaya(page);
    // Step index 1 (deal-list) has a Back button; step index 0 (login-personas) requires an
    ...
    await expect(page.getByText('Choose Maya Levin to continue.')).toBeVisible();
...
test.describe('guided tour: permission mismatch mid-flow', () => {
  test('choosing an unnarrated persona at a switch gate still degrades gracefully with no restricted-deal leakage', async ({
    page
  }) => {
    // The tour narrates "choose Nora Chen" at the settings-personas gate, but settings.tsx's
    // advanceGuidedTour('settings-personas') call fires for ANY successful persona switch, not
    // specifically Nora -- confirmed by reading apps/web/src/routes/settings.tsx. Deliberately
    // picking a different persona (Owen Patel, who cannot read the restricted deal either)
    // exercises exactly the scenario the task asked about: a persona lacking permission for a
    // step's route reaching that step.
    ...
    await page.reload();
    await expect(page.getByRole('heading', { name: 'Switch to the restricted deal owner' })).toBeVisible();

    await page.getByRole('radio', { name: /Owen Patel/ }).check();
    await page.getByRole('button', { name: 'Use selected persona' }).click();

    // Owen cannot read the restricted opportunity, so the loader redirects to /forbidden instead
    // of the narrated /deals/OPP-1003 -- no crash, no blank spotlight, and no leaked deal identity.
    ...
    await expect(page.getByText(`Step 9 of ${TOTAL_STEPS}`)).toBeVisible();
  });
});
