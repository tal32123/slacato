 ...
 */
function briefWithWarnings(warnings: readonly unknown[]): DealBrief {
  return {
    dealSnapshot: {
      accountName: 'Northstar',
      opportunityName: 'Renewal',
    ...
    executiveSummary: {
      narrative: 'The renewal is progressing.',
      claims: [authorizedClaim('claim_es_1', 'The renewal is progressing.')]
    },
...
function renderWorkspaceForNarrowerViewer(warnings: readonly unknown[]) {
  const latestRun: LatestDealRun = {
    runId: 'run-1',
    status: 'completed',
    updatedAt: '2026-08-29T02:00:00.000Z',
    generatedOutput: { lifecycle: 'finalized', brief: briefWithWarnings(warnings) }
  };
...
it('still renders a review warning whose claims cite authorized evidence', () => {
  const workspace = renderWorkspaceForNarrowerViewer([unboundWarning, boundWarning]);
  const content = workspace.generatedOutput?.content;

  expect(content?.warnings).toHaveLength(1);
  expect(content?.warnings[0]?.message).toBe('Confirm the renewal close date with the deal desk.');
  expect(content?.warnings[0]?.citationIds).toEqual([AUTHORIZED_EVIDENCE_ID]);
  expect(content?.sections.confidenceAndReviewWarnings.items).toEqual([
    'Confirm the renewal close date with the deal desk.'
  ]);
});
